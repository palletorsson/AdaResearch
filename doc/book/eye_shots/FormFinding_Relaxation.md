# Eye shot — FormFinding_Relaxation

> one pass: ride (gaze), move (place --only-improve), gate (pathfinder), voice (qfep). Field note, not a ruling.

## The ride (before)
clearance violations: **0 overlaps, 0 tight** — the law wants ≥1.2m to walk between.

## The move
    logged 1 entries to ada_run\placement_log.json
    summary: 1 maps processed
      constraint improved: 0/1  mean Δ=+0.000
      walkability improved: 0/1  mean Δ=+0.000

no sibling kept — the move did not beat the ride (overlaps 0→0, tight 0→0). Note-only.

## The voice (qfep)
4 of 4 cast members carry a theory-claim; 0 mute.
- **frozen_glass_vessel** — steps — the clock. Longer simulation = longer neck. Duration is the design parameter. Duration is DNA. The clo
- **mass_spring_bench** — "NOBODY MODELLED THIS SHAPE — THE SPRINGS DID" — form emerges from the lattice A bench-top grid of masses and 
- **science_screen** — Transformation here is projection: a 3D state is re-expressed in 2D while preserving selected structural relat
- **verlet_workbench** — "INTEGRATE POSITION FROM POSITION — THE CONSTRAINTS HOLD WITHOUT BEING SOLVED" A workbench running the actual 

## The text vs the space
walked.md exists — the writing names 3/4 of the cast; dwells declared for 0.
- space without text: science_screen — standing in the room, absent from the walk.

## The heuristic understanding
The space already walks: the bodies keep the law without being told. What carries this map is its voice, not its floor.
