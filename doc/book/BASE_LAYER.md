...
Assembled 2026-08-30T11:01:03 by `tools/build_base_layer.py`. Do not edit this file — it is derived.
...
| | |
|---|---|
| works in the book | 803 |
| speaking through an `@identity` block | 612 (76%) |
| speaking through leading prose | 66 |
| **silent — no header of their own** | **125** |
| with a wall text | 698 |
| with a reflection | 214 |
| prose assembled | 636 KB |
| documentation cut away | 1087 KB |
...
> You arrive late. No reboot, always restart.  
> The folding past keeps running.
...
> the frame counter update cycle.
...
> Point Zero, now as an object in the exhibition
...
*Reflection.* A point is that which has no part, so it has no place either: it borrows its whole existence from a coordinate system that is not itself a point.
...
> “Show me what you can do with those hands”  
>   
> Grab the ball.
...
### void crossing  <span>·</span>  `Void_Crossing`
...
### vfm 01 foundations  <span>·</span>  `VFM_01_Foundations`
...
> 3D monitor with frame displaying visualization graphics
...
### vectoroperations  <span>·</span>  `VectorOperations`
...
> The cross product does not live in the plane of its inputs
...
> The dot product does not measure length — it measures how much two directions agree
...
### vectoradvanced  <span>·</span>  `VectorAdvanced`
...
### vfm 03 motion  <span>·</span>  `VFM_03_Motion`
...
> Interactive boid flocking with VR sliders for separation
...
### composition  <span>·</span>  `ForcesComposition`
...
### vectorapplied  <span>·</span>  `VectorApplied`
...
### vfm 04 fields  <span>·</span>  `VFM_04_Fields`
...
> noise is not the absence of order but order at a length scale you have

**essence** — a small canvas across which hundreds of particles drift, each one blindly obeying the local angle of a Perlin noise field and dragging a colored trail behind it. No particle is told where to go; it only reads the field beneath its feet and steps. The image is the AGGREGATE of those steps — paint laid down by a crowd that never agreed on a destination.

**desire** — to make an invisible vector field visible by letting things FALL through it. The field has no color, no shape, no edge; only when bodies advect along it do its grain and swirls appear. It wants to show that structure can be felt before it can be seen.

**truth** — noise is not the absence of order but order at a length scale you have not yet named. The painter is that thesis enacted — randomness given a frequency becomes a current, and a current given bodies becomes a picture.

**emerges** — large scale reads CURRENT / RIVER; small scale reads TURBULENCE / STORM. Same particles, the field's frequency IS the difference between flow that reads as ordered and flow that reads as chaotic — the λ-edge made paint

**critical_parameter** — noise_scale — the spatial frequency of the field. Low and the particles sweep in broad laminar rivers; high and the field shatters into tight turbulent eddies. It is the dial between calm and chaos without ever touching the particles themselves. (noise_speed evolves the field in time so no frame's flow is the last word.)

#### Vector Field Visualizer

`vector_field_visualizer`

> 3D arrow field showing flow — vortex, source, sink
...
### systems  <span>·</span>  `ForcesSystems`
...
### vfm 06 springs  <span>·</span>  `VFM_06_Springs`
...
> Every oscillation in nature eventually damps — the question is how
...
*Reflection.* both sides run the same arithmetic, so the arena cannot test understanding, only speed - the drone knows exactly what the learner knows, and the reflex is all that is left to grade.

#### health_display

`health_display`

> A health display is not a stat — it is a promise that change has consequences and that the world keeps …
...
> There is no single way to break a sphere. Each fracture method is a decomposition basis
...
> Vector addition is not about arrows — it is about the fact that two displacements taken sequentially are equivalent to one …
