...
## Optional museum comparison. The default retains the authored large dome.
@export_enum("none", "warp") var stand: String = "none"
...
func _ready() -> void:
	...
	if stand == "warp":
		var study: Node3D = load("res://algorithms/randomness/noisesphere/warp_study.gd").new()
		study.name = "WarpStudy"
		add_child(study)
		study.build(_sphere())
		_built = true
		return
...
func _read_meta_overrides() -> void:
	if has_meta("config_stand"):
		var mode: String = str(get_meta("config_stand")).strip_edges().to_lower()
		if mode in ["none", "warp"]: stand = mode
...
func apply_grid_config(config: Dictionary) -> void:
	...
	if has_node("WarpStudy"):
		return                       # the comparison owns its fields and materials
