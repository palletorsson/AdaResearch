...
func run() -> void:
	...
	for row in spec.get("rays", []):
		var ray := PhysicsRayQueryParameters3D.create(origin + Vector3(float(row["x"]),float(row.get("from_y",20.0)),float(row["z"])), origin + Vector3(float(row["x"]),-5,float(row["z"])), 1)
...
func check_deformation(study:Node3D,report:Dictionary)->void:
	...
	for vertex in shell.displaced_vertices:
		if not shell.surface.custom_aabb.has_point(vertex):inside_bounds=false
	check(report,"displaced bounds fit expanded render bounds",inside_bounds)
