...
func _create_labels() -> void:
	...
	var title := HangarKit.signage("DISTRIBUTION SAMPLER", ["blue counts · pale model counts", "orange shape · separate scale"], Vector2(display_width * 0.90, 0.15), 0.13, Vector3(0, 1, 0))
	title.position = Vector3(0, display_height + 0.04, 0)
	add_child(title)
...
func _clear_samples(_button = null) -> void:
	...
	if _sample_markers != null:
		for marker in _sample_markers.get_children():
			_sample_markers.remove_child(marker)
			marker.queue_free()
...
func _add_sample() -> void:
	# Reserve capacity when a draw is issued, including beads still in flight.
	if _drawn >= max_samples:
		return
...
## law is computed first and then clamped into [0, 1] by `_fit`, which counts outside draws.
...
## same call, same order, same stream. Logarithmic transforms guard zero without shifting all draws.
func _rand() -> float:
...
func _update_falling_samples(delta: float) -> void:
	...
	for i in range(_falling_samples.size()):
		...
		# Land at the bar actually drawn, including its one-centimetre baseline.
		var bin_height: float = _bar_mm.get_instance_transform(bin_idx).origin.y * 2.0
	...
	for i in range(to_remove.size() - 1, -1, -1):
		...
		if idx < _sample_markers.get_child_count():
			var marker: Node = _sample_markers.get_child(idx)
			_sample_markers.remove_child(marker)
			marker.queue_free()
...
func _update_theory_curve() -> void:
	...
	# This is a shape reference, not a count axis. Keep its own peak inside
	# the panel; expected-count ghosts carry the quantitative comparison.
	var densities: Array[float] = []
	var peak: float = 0.0
	for i in range(CURVE_SEGMENTS + 1):
		var density: float = _theoretical_pdf(float(i) / float(CURVE_SEGMENTS))
		densities.append(density)
		peak = maxf(peak, density)
	for i in range(CURVE_SEGMENTS + 1):
		var x_norm := float(i) / float(CURVE_SEGMENTS)
		var y: float = densities[i] / maxf(peak, 0.0001)
...
func batch(n: int) -> void:
	var count: int = clampi(n, 0, maxi(max_samples - _drawn, 0))
