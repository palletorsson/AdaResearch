...
# desire: To be played with — slide between Koch, Sierpinski, Dragon, Plant, Bush, Fern, Binary Tree and Cantor in real time
# critical_parameter: preset (0-7) — eight fundamentally different grammars, each a different visual universe
...
# truth: Eight presets, one turtle — the grammar is the artist.
...
var lesson_enabled := false
var _batching := false
var effective_generations := 0
var budget_limited := false
var rewrite_revision := 0
var fitted_lines: Array[Vector3] = []
var fit_scale := 1.0
...
##            koch · sierpinski · dragon · plant · bush · fern · binary_tree · cantor
...
@export var angle_degrees: float = 25.0:
	set(value):
		angle_degrees = clampf(value, 5.0, 180.0)
		if _built and not _batching:
			_draw_lsystem()
			_update_info()
		_sync_angle_slider()
...
@export var generations: int = 4:
	set(value):
		generations = clampi(value, 0, 10)
		_generate()
		_sync_gen_slider()

## Current preset
@export_enum("Koch Curve", "Sierpinski", "Dragon Curve", "Plant", "Bush", "Fern", "Binary Tree", "Cantor", "Custom") var preset: int = 3:
	set(value):
		preset = clampi(value, 0, 8)
		if not _batching:
			_apply_preset()
			_sync_preset_slider()
...
@export_enum("koch", "sierpinski", "dragon", "plant", "bush", "fern", "binary_tree", "cantor") var grammar: String = "plant"
...
## Cantor (2026-09-25, from the Nature of Code reading): the interval the fractals
## sequence cuts five times, as two letters and two rules. F draws a step, f walks
## one without drawing; F -> FfF, f -> fff. After n rewrites the sentence holds
## 2^n drawn pieces in 3^n steps — the n-th row of the Cantor desk, in order,
## before any turtle has moved. No turns, so the angle is idle.
const PRESETS = {
...
func _ready():
	...
	_create_base()
	_create_labels()
	if not lesson_enabled: _create_vr_controls()
	_built = true
	_resolve_grammar()
	if lesson_enabled: _install_lesson()
...
func _create_vr_controls():
	...
	_control_panel = RackTpl.create_panel("L-SYSTEM", [
		[{"type": "slider_h", "label": "PRESET", "default": float(preset) / 7.0}],
		[
			{"type": "slider_h", "label": "GEN", "default": float(generations) / 10.0},
			{"type": "slider_h", "label": "ANGLE", "default": (angle_degrees - 5.0) / 175.0},
		],
	])
...
func _on_gen_slider_moved(_position):
	if _gen_slider and _gen_slider.has_method("get_normalized_value"):
		...
		var new_gen = clampi(int(norm * 10.99), 0, 10)
...
func _generate():
	if not _built or _batching: return
	rewrite_revision += 1
	_current_string = axiom
	effective_generations = 0
	budget_limited = _current_string.length() > max_symbols
	if budget_limited: _current_string = ""
	for _gen in range(generations):
		if budget_limited: break
		# Refuse an entire next generation before allocating it. The retained
		# sentence stays complete; a length cutoff must not cut a branch's brackets.
		var next_size := 0
		for c in _current_string:
			next_size += str(_rules.get(c, c)).length()
			if next_size > max_symbols: break
		if next_size > max_symbols:
			budget_limited = true
			break
		var new_string = ""
		for c in _current_string:
...
func _draw_lsystem():
	...
	for i in range(0, lines.size(), 2):
		...
		fitted_lines.append(p1)
		fitted_lines.append(p2)
...
func _update_info():
	...
	_info_label.text = "%s\nGen: %d/%d | Angle: %.1f°%s" % [name, effective_generations, generations, angle_degrees, " | BUDGET" if budget_limited else ""]
...
func _input(event):
	...
	if event is InputEventKey and event.pressed:
		match event.keycode:
			...
			KEY_LEFT:
				generations = maxi(0, generations - 1)
			KEY_RIGHT:
...
func apply_grid_config(config_data: Dictionary):
	if str(config_data.get("lesson", "false")).to_lower() == "true":
		lesson_enabled = true
		max_symbols = 4096
	...
	for key in config_data:
		...
		if k == "depth":
			...
			continue
		match k:
			"display_size", "max_line_length": set(k, float(config_data[key]))
			"preset": preset = int(config_data[key])
			"generations": generations = int(config_data[key])
			"angle_degrees": angle_degrees = float(config_data[key])
...
func _install_lesson() -> void:
	if has_node("GrammarLesson"): return
	var desk = load("res://commons/artifacts/grammar_lesson/grammar_lesson.gd").new()
	desk.name = "GrammarLesson"
	add_child(desk)
