...
| 16 | 9ba8d42a9 | Palle: *"that is very good, its a bit sublime, let the stones become a primitives"* → the mineral kingdom was the last layer arriving whole, so it walks the ladder now: `solid` (Primitives_Polythedra) stands ONE of the hall's polyhedra per mineral cell (cube, octahedron or prism by the cell, 0.52 x 0.72 x 0.52 m, square to the world); `subdivide` (Primitives_Portals) splits it, the pieces multiplying from 1 to 6–9, each shrinking from the whole stone toward the shard and standing on the smallest ring that keeps pieces of that width clear of one another, r = w / (2 sin(π/n)) — the first try scaled that ring by the amount instead, which collapsed a half-split stone's halves into each other so the morph read as one solid until its last frame; `translate` (Trans_Translation) is what lets the scree run, ending a generation-5 fiction where scree ran downhill on a plane with no downhill; `sample` (Random_Definition) is the tilt — yaw, offsets (off a regular ring) and every rotation mix from square to their draws | 0.990 / 0.744; at `full` every count and all 6 x organisms identical to generation 14, only timing fields differ | kept |
| 17 | 71cbf320a | **Three critics, one contract, run in parallel on generation 16** — they converged without seeing each other: the mineral kingdom has no reciprocal relation with life. Nothing is placed BY rock (`_minerals()` runs after `_ecology()`, so the rock map does not exist when living cells are chosen, and `_rock`/`_clusters` appear only as prohibitions), and the one relation the stone sends out runs backwards to its own driver: delivered scree against budget, by relief, was 0.30 5/8, 0.40 8/10, 0.50 15/15, 0.60 15/15, 0.75 6/18, 0.80 3/18 — `relief` lengthens the trail and roughens the ground that breaks it, and above 0.70 the breaking wins. Eleven of eighteen creature bodies were the family named for living near crystals; eight stood beyond the 3 m where that preference pays anything, and none of the eighteen faced a stone → the scree walks the FALL LINE (gradient re-read at each step), the lip becomes `0.015 × amplitude` instead of a flat 2 cm, and where the fall line points away from the basin gen 5's substitution is blended with the descent rather than replacing it; one creature cell is placed at a scree foot from its own `hash([seed, "rock_fauna"])` stream, appended last to the creature order so the ladder cuts it first; and that body turns to face its own cluster | scree 52 → 74 of 84; relief ≥ 0.70 delivered 0.250 → 0.917 (s13 3/18 → 15/18, s23 6/18 → 18/18), s7 and s17 each lose 2 to honest stops at the water and the edge; 6 of 6 rock bodies face their stone against 0 of 18 before; integration 0.990 → 1.000 (saturated too now), legibility 0.744 → 0.746, 4.0 % changed, every world above its own floor | kept |
...
Generation 17 changed the method as well as the object. Instead of one critic, **three ran in parallel under one identical contract**, each writing its own file and forbidden to touch anything else. They came at generation 16 by three different routes — one through the scree's arithmetic, one through the call order of `_minerals()` and `_ecology()`, one through the residents' habitat records — and landed on the same weakest thing. That agreement is worth more than any single critique, because the three could not see each other; and their disagreements were useful too, since each found a fact the others missed. The third wrote the sentence the builder was briefed from: generation 16 gave the stone a biography and left it with no neighbours.
...
- **Fan the critic out.** Three critics on one contract, run in parallel and blind to each other, converged on one weakest thing by three different routes, and each brought a fact the other two missed — the scree inversion, the call-order proof that rock is unreachable in principle, and the eleven-of-eighteen misnamed bodies. A single critic gives you one opinion and no way to tell taste from diagnosis.
- **legibility-v1's water term counts the crystals, and always has.** The test is `(b > r+18) & (b > g+8)`; the crystal albedo (0.58, 0.66, 0.86) and the scree's (0.62, 0.70, 0.88) both clear it. Re-measured over the stored tiles, the share of that number that is NOT pool: generation 0 100 %, generation 5 59 %, generations 11/14/16 86 %. So the legibility climb from 0.473 to 0.744 is substantially a story about spires. The number was NOT rewritten — that would silently restate every row above — but `pool_px`, a shade-invariant ratio test, now sits beside it as a lower bound, and the pool's true share is reported as a bracket. Third measure in this loop to turn out to be saying nothing, after integration-v1 saturated and the change percentage turned out to have a floor made of re-dithering foliage.
