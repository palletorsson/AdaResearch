The five-segment capsule made something available that its familiar name had not prepared us to notice. Now another invitation: add a division, then another. Watch what changes. **What would count as arriving?**
...
Each straight side is a chord between two points on the circle. Adding sides brings their total length closer to the circumference, which the panel labels **Circle**. The positive gap belongs to this planar comparison. It does not measure how far the entire three-dimensional portal is from perfection.[^portals-circle]
...
The next instrument makes a different approach visible. Two figures wait along a track. Before pressing **NEXT STEP**, point to where you think Achilles will go. Press once, compare, and predict again.
...
Continue to stage ten. The instrument now holds. Compare the figures with **A to limit now** on the display. The bodies may look as though they have arrived together, but Achilles' anchor still has about 5.86 millimetres to the limit. His visible body is larger than that remaining gap.

Press **RESTART**, then **PLAY**. Watch how the figures travel between the positions you previously placed one step at a time. Pause during a movement. Compare **A to limit now** with **A to limit at stage target**. One describes where the marker is; the other describes where this movement was taking it.

The program has added a way of moving between the numbers. It has also given us time to inspect it. These regularly timed display stages do not reproduce the shrinking durations of a continuous runner's journey. An infinite subdivision can have a finite total; it does not make ordinary motion impossible.

This instrument can pause while the museum continues. We have held one process still long enough to ask about it. The other processes have kept going.
...
Choose a stopping point for a use you can describe. Then name something you would return to investigate. A finite encounter can teach us something without exhausting what its rules make possible.
...
[^portals-hole]: Refining these torus meshes retains their central opening. For an idealised torus surface divided into `m × n` quadrilaterals, with opposite seams identified, `V = mn`, `E = 2mn`, `F = mn`, so `V − E + F = 0`. Triangulating each quadrilateral adds an edge and a face together. Rendering buffers may duplicate seam vertices, so their raw vertex count is a different count. A usable passage also depends on scale, placement and collision.
