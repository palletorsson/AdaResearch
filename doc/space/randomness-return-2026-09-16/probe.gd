extends SceneTree
const OUT := "res://doc/space/randomness-return-2026-09-16/"
...
func run() -> void:
	...
	if "--capture" in OS.get_cmdline_user_args():
		...
		if spatial.has(hall):
			...
			shots.append(["space",centre+Vector3(0,2.0,-distance),centre+Vector3(0,1.5,0)])
		if found.has("random_doors"):
	...
	var bench: Node3D = found.waiting_profile
	print("[waiting-visible] ",bench.visible," parent ",bench.get_parent().visible)
	player.global_position = bench.global_position+Vector3(0,5,-3)
	bench.process_mode = Node.PROCESS_MODE_INHERIT
	bench.show() # Capture camera is independent of the museum player-distance culler.
	bench.running = false; bench.replay()
	for i in range(6): bench.arrive()
	check(bench.cubes.size()==2 and bench.get_children().filter(func(c): return c is RigidBody3D).size()==2,"Two physical pickup cubes only")
	check(bench.cubes[0].has_method("pick_up"),"Restored cubes use pickup implementation")
	var waits: Array = bench.waits.duplicate()
	bench.replay()
	for i in range(6): bench.arrive()
	check(bench.waits==waits,"Arrival waits replay exactly")
	check(waits.all(func(v):return v>=0.3 and v<=1.3),"Waits stay in authored interval")
	check(bench.profiles.size()==5,"Five landscape layers")
	for i in range(bench.profiles.size()):
		var layer: Node3D = bench.profiles[i]
		check(layer.profile_points.size()==17,"Layer %d has seventeen samples" % i)
		check(layer.profile_points[0].y==1 and layer.profile_points[-1].y==1,"Layer %d endpoints remain pinned" % i)
		check(is_equal_approx(layer.position.y+1.0,1.7),"Layer %d horizon at eye level" % i)
		check(is_equal_approx(layer.position.z,i*0.3),"Layer %d recedes along positive Z" % i)
	await create_timer(0.8).timeout
	check(bench.cubes[0].position.y<2.0,"Pickup cube actually descends under gravity")
	await capture_state(em,bench.global_position+Vector3(0,2,-5),bench.global_position+Vector3(0,1,0),"waiting-profile")
