...
func build(remover: Node3D) -> void:
	...
	panel = rack.create_panel("CHOOSE THE SET, THEN REMOVE", [
		...
		[{"type":"button", "label":"REMOVE ONE"}, {"type":"button", "label":"RESET"}]
	])
	panel.name = "Controls"
...
func _update_status() -> void:
	...
	_status.text = "%s\nEligible: %d   Remaining: %d   Removed: %d" % [region, state["eligible_at_start"], state["remaining"], state["removed"]]
	var index: int = int(state["last_selected"] if state["busy"] else state["last_removed"])
	_last.text = "Amber: can be chosen. Grey: outside the set.\nMode buttons and RESET restore all 64 cubes."
	if index >= 0:
	...
	elif int(state["remaining"]) == 0 and bool(state["bound"]):
		_last.text = "No eligible cubes. RESET or choose another set."
