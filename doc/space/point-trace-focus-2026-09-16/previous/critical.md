...
The word *trace* below sometimes names a lived gesture and sometimes its inscription. Keep them distinct in the working encounter: the pen records selected positions, not intention or a complete duration. Its 5 mm movement gate, optional recording grid and finite capacity are separate decisions. A coarse record can also be chosen as a drawing instrument.

## The Line is the Trace, Compressed

From line_axioms: "The Line is the Trace, compressed to its endpoints."

The sentence proposes one reduction: retain only the endpoints of a recorded path. Whether that reduction harms or helps depends on what someone is trying to preserve and who can contest the choice.

A lived gesture involves more than its recorded trace:
...
- It involved **intention** - which the position record alone cannot establish
...
What is gained:
- **Calculation cost** - endpoint distance does not require visiting every stored sample
- **Storage efficiency** - 2 points instead of N samples
- **Reproducibility** - both endpoint pairs and sampled traces can be copied as data
...
A sampled trace can retain a history of positions that an endpoint pair omits. A visitor may want that history to say:
...
2. The tool inspecting its available position on process callbacks
3. Retaining positions admitted by movement, shaping and duplicate rules
4. Creating a **visible record of embodied action**

A trace needs provenance before it can count as evidence of bodily passage. The same array format can hold tracked positions, a synthetic demonstration or a copied record. The telemetry panels make that distinction explicit through their source labels.

## Sampling: The Digital Compromise

The trace is already sampled. The running frame cadence limits opportunities to read a position; the pen's movement and spatial rules determine which readings it keeps. A fixed ninety accepted points per second is not the contract of this artifact.
...
This recorder cannot decide what happened between its admitted samples. Increasing resolution changes the record; it does not supply intention or make interpretation unnecessary.
...
## The authority of a legible instrument

In the revised encounter, eighteen retained points can be displayed beside ten coordinate rows. The count and the excerpt are both accurate. Their agreement does not mean that the display shows everything the recorder holds. The casing makes the account easier to read and gives it the appearance of a finished instrument; its claims still have to be compared with the line.

The method is to move between these descriptions. Ask where row 1 has gone, inspect the retained trace, then read the array operation. Ask what a pause contributes, watch the count, then inspect the movement gate. Neither the attractive display nor the code can answer the whole question alone.

Coarse recording also offers a practice: a hand can wander within a rounding region while one stored address holds. A visitor can use that tolerance to repeat a shape or keep a chosen place steady. This use does not erase the lost differences. It changes which differences the drawing needs, and leaves that choice open to another visitor.
