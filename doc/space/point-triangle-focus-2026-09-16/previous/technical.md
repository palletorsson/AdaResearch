...
This room follows Point_Line_Grid directly. The two principal artifacts separate boundary closure from visible fill, then compare the triangle fan on the same concave outline from different starting points. Three non-collinear points define a plane; the tutorial's rigidity examples additionally assume fixed edge lengths. Dragging a vertex changes those lengths.

## Installed primary encounters

`triangle_line_puzzle` validates three lines at the shared targets with its connected/closed-loop constraints. Closure still leaves the centre empty. The side panel's SHOW / HIDE FILL button refuses an open puzzle and, after completion, creates a triangle mesh from the target positions. Its translucent material disables back-face culling. Hiding the mesh leaves the completed edge assembly in place.

`draw_triangle_faces` uses a 0.1 m world grid, a one-second held-placement interval and a 0.15 m travel threshold. Release can place a final point and always ends held sampling. The next pickup resumes the path unless NEW OUTLINE was pressed. NEW OUTLINE retains completed faces and corners while allowing a new first point.

The side instrument displays current path entries, first-corner number, completed loops and the sum of their fan triangles. A stored loop of n corners contributes n-2 triangles; front/back rendering copies are not counted as separate geometric triangles. Each editable corner carries its one-based stored index; START identifies the current path's first entry.

SHOW / HIDE FILL toggles completed meshes while preserving their boundary lines. The triangle fan itself remains unchanged: it uses the first loop point with successive pairs. The six-corner L comparison in final.md yields four triangles per loop, but starting at B makes some extend outside the intended region.

Placed positions are world coordinates. Editable handles now use top-level transforms and global_position for creation and release, matching the stored arrays and generated meshes even when the museum translates or rotates the artifact. Editing one vertex rebuilds the boundaries and fills that refer to it.

Both panels are enabled by `#discovery:1` in this room's map tokens. They use the existing hand-poke/ray-pointer buttons and shared instrument casing. No alternate triangulation method or undo control has been added.
...
When you grab a vertex sphere in the `interactivetriangle` artifact and drag it, notice how the other two vertices stay fixed. The surface reshapes to that one constraint. The triangle yields — but only to what you explicitly move. Nothing deforms implicitly. That editor is changing edge lengths; it does not demonstrate rigidity under fixed-length constraints.

---

## Constructing a Triangle in GDScript

A triangle is three vertices and a normal. Vertex winding establishes front/back orientation for culling; supplied normals support lighting. An unshaded face can be rendered without supplying lighting normals.
...
A connected strip can describe 24 triangles using 26 shared source positions instead of 72 independent corners. The renderer may still emit separate vertices for face colours and hard normals; shared source data and GPU vertex storage are different decisions.

The `folded_strip` artifact starts with 24 connected triangles. Its two rows have different heights, but both rows lie in one plane. This tilts the starting strip; it does not yet make a fold:
...
This is the topological constraint made tactile. When you grab vertex 12 in the middle of the strip and pull it sideways, triangles 10, 11 and 12 all respond. You can't move a shared vertex without affecting everything it's part of. Mesh topology is the constraint set. The geometry follows.

The winding of alternate triangles is reversed so their normals agree on the flat surface. The double-sided material displays one submitted face from either side; coincident front/back copies are unnecessary.

This hall enables `#study:1`: flat → one 70-degree hinge → flat → paired 40-degree hinges → flat, over 48 seconds. The angles vary smoothly. At each update, all vertices beyond a shared edge rotate rigidly around it; interpolating whole vertex arrays instead would generally change edge lengths.

All 26 source positions remain grabbable. A pickup pauses the study; moving a corner rebuilds its mesh and may change triangle edge lengths. Release preserves the edit. The physical **REPLAY** button restarts only after every handle has been released. Other map placements remain manual unless they explicitly opt into the study.
...
**triangle_strip_wave** — A folded strip with a sinusoidal height driver that animates the vertex heights automatically, showing how the strip topology propagates waveforms across the mesh. The current hall already animates rigid hinge folds. A sine-driven variant would add a different comparison: deformation that can stretch the edges, with that change made explicit.