...
## Per-tracker stick rest offsets, written by a calibrator
## (commons/player/joystick_deadzone_fix.gd samples the resting stick at boot).
## A worn stick reports a non-zero centre and a shrunken throw; the old answer
## was a 0.5 deadzone wall, which a sufficiently worn stick can no longer
## climb at all. Empty by default — stock behavior unless a calibrator wrote.
var stick_rest_offsets: Dictionary = {}


## One axis recentred around its calibrated rest point, rescaled by the range
## that remains in that direction — a stick resting at +0.42 still reaches the
## full -1..1 after compensation.
static func _recenter_axis(v: float, rest: float) -> float:
	var d := v - rest
	var axis_range := (1.0 - rest) if d > 0.0 else (1.0 + rest)
	return clampf(d / maxf(axis_range, 0.05), -1.0, 1.0)
...
func get_adjusted_vector2(p_controller, p_input_action):
	...
	var off_v: Variant = stick_rest_offsets.get(str(p_controller.tracker), null)
	if off_v is Vector2:
		var off: Vector2 = off_v
		original_vector = Vector2(
			_recenter_axis(original_vector.x, off.x),
			_recenter_axis(original_vector.y, off.y))
