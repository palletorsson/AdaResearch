...
func load_data(map_name: String) -> void:
	...
	if ResourceLoader.exists(interactable_path):
		...
		push_error("GridInteractableHandler: Interactable data script not found: %s" % interactable_path)
...
func apply_data() -> void:
	...
	for z in range(min(grid_system.grid_z, interactable_layout.size())):
		...
		for x in range(min(grid_system.grid_x, row.size())):
			...
			if algorithm_id != GridCommon.EMPTY_SPACE and algorithm_id != '':
				#print("GridInteractableHandler: Found algorithm ID: '%s' at position (%d, ?, %d)" % [algorithm_id, x, z])
				
				var algorithm_data = grid_system.algorithm_registry.get_algorithm(algorithm_id)
				if algorithm_data and algorithm_data.size() > 0:
					...
					#print("GridInteractableHandler: Placing algorithm '%s' at position (%d, %d, %d)" % [algorithm_id, x, y_pos, z])
					_place_algorithm(x, y_pos, z, algorithm_id, total_size)
					interactable_count += 1
				else:
					print("WARNING: GridInteractableHandler: Algorithm ID '%s' not found in registry" % algorithm_id)
	
	#print("GridInteractableHandler: Added %d interactable objects to the grid" % interactable_count)
