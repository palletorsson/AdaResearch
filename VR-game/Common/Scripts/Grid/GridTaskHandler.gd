...
func _load_global_task_definitions() -> void:
	...
	if ResourceLoader.exists(GridCommon.TASK_DATA_PATH):
		var task_data_script = load(GridCommon.TASK_DATA_PATH)
		if task_data_script:
			var task_data_instance = task_data_script.new()
			
			# Check if TASKS_JSON exists using get()
			if task_data_instance.get("TASKS_JSON") != null:
				task_json_data = task_data_instance.TASKS_JSON
				#print("GridTaskHandler: Loaded %d task definitions from %s" % [task_json_data.size(), GridCommon.TASK_DATA_PATH])
				...
				push_error("GridTaskHandler: TASKS_JSON constant not found in task data script")
			
			# Clean up the instance
			if task_data_instance.has_method("queue_free"):
				task_data_instance.queue_free()
		else:
			push_error("GridTaskHandler: Failed to instantiate task data script")
	else:
		push_error("GridTaskHandler: Task data script not found at %s" % GridCommon.TASK_DATA_PATH)

# Load map-specific task layout from the map's task_data.gd file
func _load_map_specific_task_layout(map_name: String) -> void:
	var map_task_data_path = GridCommon.MAPS_PATH + map_name + "/task_data.gd"
	
	if ResourceLoader.exists(map_task_data_path):
		var map_task_data_script = load(map_task_data_path)
		if map_task_data_script:
			var map_task_data_instance = map_task_data_script.new()
			
			# Check if task_data array exists using get()
			if map_task_data_instance.get("task_data") != null:
				task_layout_data = map_task_data_instance.task_data
				#print("GridTaskHandler: Loaded map-specific task layout from %s" % map_task_data_path)
				#print("GridTaskHandler: Task layout has %d rows" % task_layout_data.size())
			else:
				push_error("GridTaskHandler: 'task_data' array not found in map task data script")
			
			# Clean up the instance
			if map_task_data_instance.has_method("queue_free"):
				map_task_data_instance.queue_free()
		else:
			push_error("GridTaskHandler: Failed to instantiate map task data script")
	else:
		print("GridTaskHandler: No map-specific task layout found at %s, using global positions instead" % map_task_data_path)
...
func _create_default_task_marker(task_data: Dictionary) -> MeshInstance3D:
	...
	label.text = task_data.get("task_name", "Unknown Task")
