...
func _load_task_data():
	if ResourceLoader.exists(TASK_DATA_PATH):
		var task_data_script = load(TASK_DATA_PATH)
		if task_data_script:
			var task_data_instance = task_data_script.new()
			print("TaskSystem: Loaded task data from %s" % TASK_DATA_PATH)
			return task_data_instance
		else:
			push_error("TaskSystem: Failed to load task data script: %s" % TASK_DATA_PATH)
	else:
		push_error("TaskSystem: Task data script not found: %s" % TASK_DATA_PATH)
...
func get_tasks_json():
	if _tasks_data and _tasks_data.has_constant("TASKS_JSON"):
		return _tasks_data.TASKS_JSON
	return []
