...
func create_3d_maze():
	...
	floor_instance.position = Vector3(grid_width * cell_size / 2, 0, grid_depth * cell_size / 2)
	add_child(floor_instance)
	
	# Create maze walls
	for z in range(grid_depth):
		for x in range(grid_width):
			var wall_position = Vector3(x * cell_size, 0, z * cell_size)
...
func setup_camera():
	...
	# Position the camera to view the maze
	camera_pivot.position = Vector3(grid_width * cell_size / 2, 0, grid_depth * cell_size / 2)
	camera.position = Vector3(0, grid_width * cell_size * 0.8, grid_depth * cell_size * 0.8)
	camera.look_at(Vector3(grid_width * cell_size / 2, 0, grid_depth * cell_size / 2))
