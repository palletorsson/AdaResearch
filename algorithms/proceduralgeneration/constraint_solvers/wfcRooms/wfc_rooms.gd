...
func _build_tile(name: String, sockets: Dictionary) -> Node3D:
	...
	mat_floor.albedo_color = Color(0.2, 0.22, 0.25)
	mat_floor.roughness = 0.2
	
	var mat_wall := StandardMaterial3D.new()
	mat_wall.albedo_color = Color(0.8, 0.82, 0.8)
	mat_wall.roughness = 0.8

	# floor slab
	var floor_min := Vector3(0.0, 0.0, 0.0)
	var floor_max := Vector3(TILE_SIZE, FLOOR_THICK, TILE_SIZE)
	var floor_box = _add_box_csg("Floor", floor_min, floor_max, mat_floor)
	...
	for side in ["N","E","S","W"]:
		...
		if state == "wall":
			wall_count += 1
			_place_wall_edge(tile, side, false, mat_wall)
		elif state == "door":
			wall_count += 1
...
func _place_wall_edge(parent: Node3D, side: String, with_door: bool, material: Material) -> void:
	...
	match side:
		"N":
			if not with_door:
				parent.add_child(_add_box_csg("Wall_N", Vector3(x0, z0, y1 - t), Vector3(x1, z1, y1), material), true)
			else:
				parent.add_child(_add_box_csg("Wall_N_L", Vector3(x0,        z0, y1 - t), Vector3(x0 + seg, z1, y1), material), true)
				parent.add_child(_add_box_csg("Wall_N_R", Vector3(x1 - seg,  z0, y1 - t), Vector3(x1,       z1, y1), material), true)
				if head > 0.001:
					parent.add_child(_add_box_csg("Lintel_N",
						Vector3(x0 + seg, z0 + DOOR_HEIGHT, y1 - t),
						Vector3(x1 - seg, z1,              y1), material), true)
		"S":
			if not with_door:
				parent.add_child(_add_box_csg("Wall_S", Vector3(x0, z0, y0), Vector3(x1, z1, y0 + t), material), true)
			else:
				parent.add_child(_add_box_csg("Wall_S_L", Vector3(x0,        z0, y0), Vector3(x0 + seg, z1, y0 + t), material), true)
				parent.add_child(_add_box_csg("Wall_S_R", Vector3(x1 - seg,  z0, y0), Vector3(x1,       z1, y0 + t), material), true)
				if head > 0.001:
					parent.add_child(_add_box_csg("Lintel_S",
						Vector3(x0 + seg, z0 + DOOR_HEIGHT, y0),
						Vector3(x1 - seg, z1,              y0 + t), material), true)
		"E":
			if not with_door:
				parent.add_child(_add_box_csg("Wall_E", Vector3(x1 - t, z0, y0), Vector3(x1, z1, y1), material), true)
			else:
				parent.add_child(_add_box_csg("Wall_E_B", Vector3(x1 - t, z0, y0),        Vector3(x1, z1, y0 + seg), material), true)
				parent.add_child(_add_box_csg("Wall_E_T", Vector3(x1 - t, z0, y1 - seg),  Vector3(x1, z1, y1), material), true)
				if head > 0.001:
					parent.add_child(_add_box_csg("Lintel_E",
						Vector3(x1 - t, z0 + DOOR_HEIGHT, y0 + seg),
						Vector3(x1,     z1,              y1 - seg), material), true)
		"W":
			if not with_door:
				parent.add_child(_add_box_csg("Wall_W", Vector3(x0, z0, y0), Vector3(x0 + t, z1, y1), material), true)
			else:
				parent.add_child(_add_box_csg("Wall_W_B", Vector3(x0, z0, y0),        Vector3(x0 + t, z1, y0 + seg), material), true)
				parent.add_child(_add_box_csg("Wall_W_T", Vector3(x0, z0, y1 - seg),  Vector3(x0 + t, z1, y1), material), true)
				if head > 0.001:
					parent.add_child(_add_box_csg("Lintel_W",
						Vector3(x0,     z0 + DOOR_HEIGHT, y0 + seg),
						Vector3(x0 + t, z1,              y1 - seg), material), true)

func _add_box_csg(name: String, min_v: Vector3, max_v: Vector3, material: Material) -> CSGBox3D:
