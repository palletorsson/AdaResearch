...
@export var stage_color: Color = Color(0.95, 0.93, 0.88)  # Off-white
...
func generate_bsp():
	...
	# Create root node - offset upward so BSP sits above the stage
	var offset_y = space_size.y * 0.1  # Lift 10% of height
	var initial_bounds = AABB(Vector3(-space_size.x / 2, -space_size.y / 2 + offset_y, -space_size.z / 2), space_size)
	...
	# Visualize - stage first, then partitions (which no longer clears)
	if show_stage:
		create_stage()
	if show_partitions:
...
func create_stage():
	...
	stage.name = "Stage"
	add_child.call_deferred(stage)

	var box_mesh = BoxMesh.new()
	box_mesh.size = Vector3(space_size.x * 1.3, 0.3, space_size.z * 1.3)
	stage.mesh = box_mesh

	# Position below the BSP visualization
	stage.position.y = -space_size.y / 2 - 0.15
	...
	# Add a subtle rim/edge to the stage
	var rim = MeshInstance3D.new()
	rim.name = "StageRim"
	add_child.call_deferred(rim)

	var rim_mesh = BoxMesh.new()
	rim_mesh.size = Vector3(space_size.x * 1.35, 0.05, space_size.z * 1.35)
	rim.mesh = rim_mesh
	rim.position.y = -space_size.y / 2 + 0.025

	var rim_mat = StandardMaterial3D.new()
	rim_mat.albedo_color = Color(0.85, 0.82, 0.78)  # Slightly darker rim
