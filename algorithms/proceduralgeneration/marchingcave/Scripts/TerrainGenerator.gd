...
func init_compute() -> bool:
	...
	if lut.is_empty():
		push_error("TerrainGenerator: LUT data is empty, cannot create buffer")
		return false
	
	var lut_bytes = PackedInt32Array(lut).to_byte_array()
	if lut_bytes.is_empty():
		push_error("TerrainGenerator: LUT byte array is empty")
		return false
	
	lut_buffer = rendering_device.storage_buffer_create(lut_bytes.size(), lut_bytes)
	if not lut_buffer.is_valid():
		push_error("TerrainGenerator: Failed to create LUT buffer")
		return false
...
func load_lut(file_path):
	...
	print("TerrainGenerator: Successfully loaded " + str(indices.size()) + " LUT indices from file")
	return indices
...
func get_embedded_lut() -> Array:
	...
	print("TerrainGenerator: Using embedded LUT data (" + str(indices.size()) + " indices)")
	return indices
