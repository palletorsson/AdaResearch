...
{   
	...
	// GYROID Implemention with Randomness
	
	// Base scale of the simulation
	float baseScale = 0.5;
	vec3 p = worldPos * baseScale;
	
	// 1. DOMAIN WARPING
	// We warp the coordinate space itself with large scale noise
	// This makes the gyroid twist and bend like melted wax
	float warpScale = 0.1; // Large, smooth distortions
	vec3 warpOffset = vec3(params.noiseOffsetX, params.noiseOffsetY, params.noiseOffsetZ);
	
	vec3 warp = vec3(
		snoise((p + warpOffset) * warpScale),
		snoise((p + warpOffset + vec3(52.1, 13.7, 91.4)) * warpScale),
		snoise((p + warpOffset + vec3(-19.3, 44.2, 12.8)) * warpScale)
	);
