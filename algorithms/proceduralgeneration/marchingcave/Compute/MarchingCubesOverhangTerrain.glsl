...
// Flat terrain with overhangs, caves, and interesting topology
...
{   
	...
	float baseTerrain = (surfaceHeight - worldPos.y) / 30.0;  // Inverted: below = positive (solid)
	
	// ===== OVERHANG FEATURES =====
	// Calculate 3D noise for creating overhangs and rocky outcrops
	float overhangNoise = 0;
	float amplitude = 1.0;
	float frequency = 1.0;
	
	for (int i = 0; i < 4; i++)
	{
		float noise = snoise(samplePos * frequency);
		overhangNoise += noise * amplitude;
		frequency *= 2.0;
		amplitude *= 0.5;
	}
	...
	overhangStrength *= mix(0.3, 1.0, heightMod);
	
	// Apply overhangs - subtracts from density to create air pockets
	float overhangDensity = overhangNoise * overhangStrength;
	
	// ===== CAVE ENTRANCES =====
	// Carve cave entrances at specific locations using ridged noise
	float caveNoise = 0;
	amplitude = 1.0;
	frequency = 0.5;
	...
	for (int i = 0; i < 3; i++)
	{
		float noise = snoise(samplePos * frequency + vec3(100.0, 0, 50.0));
		noise = 1.0 - abs(noise); // Ridged noise
		noise *= noise;
		caveNoise += noise * amplitude;
		frequency *= 2.5;
		amplitude *= 0.4;
	}
	...
	float archNoise = 0;
	vec3 archSample = samplePos * 0.8 + vec3(50.0, 0, -50.0);
	
	// Horizontal banding for arch layers
	float horizontalBands = sin(worldPos.y * 0.15) * 0.5 + 0.5;
	
	// Vertical pillars
	float pillarNoise = snoise(vec3(worldPos.x * 0.05, 0, worldPos.z * 0.05));
	pillarNoise = abs(pillarNoise);
	
	// Combine for arch effect - solid pillars with air between
	archNoise = pillarNoise * horizontalBands;
	
	// Only apply arches in specific areas
	...
	float outcroppNoise = snoise(samplePos * 1.5 + vec3(0, 100.0, 0));
	outcroppNoise = max(0.0, outcroppNoise); // Only positive values
	outcroppNoise *= outcroppNoise; // Sharpen
	...
	float density = baseTerrain 
	              - overhangDensity 
	              - caveStrength 
	              - archStrength 
	              + outcropStrength;  // Changed to + for adding solid outcrops
