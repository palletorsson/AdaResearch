...
func _on_pattern_selected(index) -> void:
...
func _on_spacing_changed(value) -> void:
	net_node.spacing = value
	$Panel/VBoxContainer/SpacingLabel.text = "Spacing: %.2f" % value

func _on_spacing_drag_ended(value_changed) -> void:
	if value_changed:
		net_node.generate_16cell_space()

func _on_hollow_toggled(button_pressed) -> void:
	net_node.create_hollow_center = button_pressed
	net_node.generate_16cell_space()
	_update_stats()

func _on_spiral_toggled(button_pressed) -> void:
