...
{   
	...
	// Create scrolling effect for the fountain
	vec3 flowSpeed = vec3(0.0, -1.0, 0.0); // Water flowing down
	vec3 animatedPos = worldPos + flowSpeed * params.time * 5.0; // Speed up the flow
	
	vec3 samplePos = (animatedPos + noiseOffset) * params.noiseScale / params.scale;
	...
	// Fountain shape: Cylinder/Column
