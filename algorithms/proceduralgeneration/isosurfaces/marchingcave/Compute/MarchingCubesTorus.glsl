...
{   
	...
	// Torus parameters
	float majorRadius = 50.0;  // Distance from center to tube center
	float minorRadius = 20.0;   // Tube thickness
	
	// Calculate torus SDF (Signed Distance Function)
	// First, get distance from Y axis in XZ plane
	float distFromYAxis = length(vec2(worldPos.x, worldPos.z));
	
	// Then get 2D distance to the torus ring
	vec2 torusPoint = vec2(distFromYAxis - majorRadius, worldPos.y);
	float torusDist = length(torusPoint) - minorRadius;
	
	// Add organic noise deformation for sculptural quality
	float deformation = 0;
	float amplitude = 1.0;
	float frequency = 1.0;
	
	// Multiple octaves of noise for organic surface
	for (int i = 0; i < 5; i++)
	{
	...
	// Add twisting along the torus
	float angle = atan(worldPos.z, worldPos.x);
	float twist = sin(angle * 3.0 + worldPos.y * 0.1) * 3.0;
