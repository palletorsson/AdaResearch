...
{   
	...
	// Calculate cave noise (3D pockets)
	float caveNoise = 0;
	float amplitude = 1;
	float frequency = 1;
	
	// Use fewer octaves and less aggressive noise for occasional caves
	for (int i = 0; i < 4; i++)
	{
		float noise = snoise(samplePos * frequency) * 2 - 1;
		noise = 1 - abs(noise);
		noise *= noise;
	...
	// Add some surface variation (rolling hills)
	...
	// Terrain is solid below surface, air above
