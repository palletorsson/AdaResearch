...
layout(set = 0, binding = 1, std430) restrict buffer ParamsBuffer
{
	...
	float noiseOffsetY;
	float noiseOffsetZ;
	float shapeId; // Unused in this shader? Or maybe useful
	float numBlobs; // NEW: Number of active blobs
} params;
...
layout(set = 0, binding = 4, std430) restrict buffer BlobBuffer { Blob data[]; } blobs; // NEW: Blob array
...
float getBlobsSDF(vec3 p) {
	float dist = 1000.0;
	int count = int(params.numBlobs);
	if (count == 0) return dist;
	
	// Smoothness factor (k) — scales with volume, lower = more defined blobs
	float k = params.scale * 0.06;
	
	for (int i = 0; i < count; i++) {
		vec4 blob = blobs.data[i].pos_rad;
		float d = sdSphere(p - blob.xyz, blob.w);
		
		if (i == 0) {
			dist = d;
		} else {
			dist = opSmoothUnion(dist, d, k);
		}
	}
	return dist;
}
...
{   
	...
	// SDF Evaluation
	float dist = getBlobsSDF(worldPos);
