...
func animate_koch_curve() -> void:
	"""Animate the Koch curve - VR-safe."""
	
	if not koch_mesh_instance or not koch_mesh_instance.mesh:
		return
	
	# Gentle pulsing animation
	var pulse = 1.0 + sin(time * 2.0) * 0.1
	koch_mesh_instance.scale = Vector3.ONE * pulse
	
	# Color animation
	if koch_material:
		var emission_intensity = 1.0 + sin(time * 3.0) * 0.5
		koch_material.emission_energy_multiplier = emission_intensity

func animate_indicators() -> void:
	"""Animate iteration and complexity indicators with single meshes."""
	
	# Create iteration control mesh (cylinder height based on iteration)
	var iter_height = (current_iteration + 1) * 4 + 5
	create_cylinder_mesh(iteration_mesh_instance, 2.0, iter_height)
	iteration_mesh_instance.position.y = -30 + iter_height/2
	
	# Create complexity indicator mesh (cylinder height based on segments)
	var complexity = total_segments
	var max_complexity = pow(4, max_iterations) * 3.0  # Theoretical maximum for segments
	var complexity_height = (float(complexity) / max_complexity) * 30.0 + 5.0
	create_cylinder_mesh(complexity_mesh_instance, 2.0, complexity_height)
	complexity_mesh_instance.position.y = -30 + complexity_height/2
	
	# Pulsing effects
	var pulse = 1.0 + sin(time * 4.0) * 0.1
	iteration_mesh_instance.scale.x = pulse
	complexity_mesh_instance.scale.x = pulse
...
func create_cylinder_mesh(mesh_instance: MeshInstance3D, radius: float, height: float) -> void:
	...
	cylinder_mesh.height = height
	   # Lower segments for VR performance
