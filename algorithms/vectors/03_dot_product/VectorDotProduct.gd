...
func _ready():
	...
	angle_label = create_info_panel("Î¸", Vector3(0.0, 1.6, 0.0))
...
func _process(_delta):
	...
	angle_label.text = "Î¸ â‰ˆ %.1fÂ°" % rad_to_deg(theta)
...
func _update_info(a: Vector3, b: Vector3, dot: float, proj: Vector3, rej: Vector3, theta: float, cos_theta: float):
	...
	builder.append("a Â· b = %.2f" % dot)
	builder.append("|a||b|cosÎ¸ = %.2f" % (a.length() * b.length() * cos_theta))
