...
func create_ui() -> void:
	...
	FORCES_UI.style_title_label(info_label, Vector3(0, 0.65, 0), 28)
	add_child(info_label)

	instructions_label = Label3D.new()
	FORCES_UI.style_instruction_label(instructions_label, Vector3(0, 0.55, 0), 18)
	FORCES_UI.set_label_text(instructions_label, "[F] Force arrows  |  [R] Reset")
...
func update_info_label() -> void:
	if info_label:
		FORCES_UI.set_label_text(info_label, "Example 2.2: Forces with mass\nWind %.2f  |  Drag %.3f" % [wind_strength, drag_coefficient])
