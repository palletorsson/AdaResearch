...
var text = '''
[center][b]Variational Autoencoders (VAEs)[/b][/center]
[center][i]Inside Variational Autoencoders (VAEs)[/i][/center]

This tutorial card summarizes how `res://algorithms/machinelearning/variational_autoencoders_VAEs/VariationalAutoencodersVAEs.gd` orchestrates the scene so you can teach the concept without leaving VR.
...
- `[code]create_input_particles()[/code]` handles a key part of the simulation loop.
