...
var text = '''
[center][b]Long Short-Term Memory (LSTMs)[/b][/center]
[center][i]Inside Long Short-Term Memory (LSTMs)[/i][/center]

This tutorial card summarizes how `res://algorithms/machinelearning/LSTMs/LSTMs.gd` orchestrates the scene so you can teach the concept without leaving VR.
...
- `[code]create_input_tokens()[/code]` handles a key part of the simulation loop.
