...
var text = '''
[center][b]Convolutional Neural Networks (CNNs)[/b][/center]
[center][i]Inside Convolutional Neural Networks (CNNs)[/i][/center]

This tutorial card summarizes how `res://algorithms/machinelearning/convolutional_neural_networks_CNNs/ConvolutionalNeuralNetworksCNNs.gd` orchestrates the scene so you can teach the concept without leaving VR.
...
- `[code]create_image_grids()[/code]` handles a key part of the simulation loop.
