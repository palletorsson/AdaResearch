...
@export var arrival_radius: float = 0.2
...
func _setup_environment() -> void:
	...
	force_controller.max_value = 0.3
	...
	radius_controller.parameter_name = "Arrival Radius"
	radius_controller.min_value = 0.05
	radius_controller.max_value = 0.4
	radius_controller.default_value = arrival_radius
...
func _spawn_vehicle_and_target() -> void:
	...
	_target.position = Vector3(0.3, 0.6, 0)
...
func _process(delta: float) -> void:
	...
	var dist := _vehicle.position.distance_to(_target.position)
	_status_label.text = "Arrive + Seek | Dist %.2f" % dist
...
class Vehicle:
	...
	var max_force: float = 0.12
	var arrival_radius: float = 0.2
	...
	func arrive(target: Vector3) -> void:
		...
		if d > 0:
			...
			if d < arrival_radius:
				var m := remap(d, 0, arrival_radius, 0, max_speed)
				desired *= m
			else:
				desired *= max_speed
