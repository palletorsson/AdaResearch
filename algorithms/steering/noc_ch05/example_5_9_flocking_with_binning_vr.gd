# ===========================================================================
# NOC Example 5.9: Flocking with Binning
...
@export var num_boids: int = 40
@export var alignment_weight: float = 1.0
@export var cohesion_weight: float = 1.0
@export var separation_weight: float = 1.5
@export var bin_size: float = 0.2
...
func _spawn_flock() -> void:
	_flock = Flock.new(_sim_root, MAT_AGENT, num_boids, bin_size)

func _process(delta: float) -> void:
	_flock.run(delta, alignment_weight, cohesion_weight, separation_weight)
	_status_label.text = "Flocking w/ Binning | %d boids" % _flock.boids.size()
...
class Flock:
	var boids: Array[FlockingBoidWithBinning] = []
	var bin_size: float
	var bins: Dictionary = {}

	func _init(parent: Node3D, mat: Material, count: int, b_size: float) -> void:
		bin_size = b_size
		for i in count:
			var b := FlockingBoidWithBinning.new()
			b.init(parent, mat)
			b.position = Vector3(
				randf_range(-0.4, 0.4),
				randf_range(0.1, 0.9),
	...
	func run(delta: float, align_w: float, cohesion_w: float, sep_w: float) -> void:
		_update_bins()
		for boid in boids:
			var neighbors := _get_neighbors(boid)
			boid.flock(neighbors, align_w, cohesion_w, sep_w)
			boid.update(delta)
			boid.wrap_bounds()

	func _update_bins() -> void:
		bins.clear()
		for boid in boids:
			var bin_key := _get_bin_key(boid.position)
			if not bins.has(bin_key):
				bins[bin_key] = []
			bins[bin_key].append(boid)

	func _get_bin_key(pos: Vector3) -> Vector2i:
		return Vector2i(
			int(floor(pos.x / bin_size)),
			int(floor(pos.y / bin_size))
		)

	func _get_neighbors(boid: FlockingBoidWithBinning) -> Array[FlockingBoidWithBinning]:
		var neighbors: Array[FlockingBoidWithBinning] = []
		var bin_key := _get_bin_key(boid.position)
		for dx in range(-1, 2):
			for dy in range(-1, 2):
				var check_key := Vector2i(bin_key.x + dx, bin_key.y + dy)
				if bins.has(check_key):
					neighbors.append_array(bins[check_key])
		return neighbors
...
class FlockingBoidWithBinning:
	...
	func flock(neighbors: Array[FlockingBoidWithBinning], align_w: float, cohesion_w: float, sep_w: float) -> void:
		var sep := separate(neighbors) * sep_w
		var ali := align(neighbors) * align_w
		var coh := cohesion(neighbors) * cohesion_w
		apply_force(sep)
		apply_force(ali)
		apply_force(coh)

	func separate(boids: Array[FlockingBoidWithBinning]) -> Vector3:
	...
	func align(boids: Array[FlockingBoidWithBinning]) -> Vector3:
	...
	func cohesion(boids: Array[FlockingBoidWithBinning]) -> Vector3:
		var sum := Vector3.ZERO
		var count := 0
		for other in boids:
			if other == self:
