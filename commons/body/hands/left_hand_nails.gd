...
# Script to apply saved nail color from GameManager to left hand nails
...
func _find_nail_mesh() -> void:
	...
	# Try to find left hand nail mesh relative to parent
	var possible_paths = [
	...
	if not nail_mesh:
		push_warning("LeftHandNails: Could not find nail mesh")
