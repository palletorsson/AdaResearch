var text = '''[b]Waves & Trigonometry[/b]
[i]The Unit Circle and Wave Functions[/i]

The unit circle is a circle with a radius of 1 centered at the origin (0,0) in the Cartesian coordinate system.

It provides a geometric model that helps visualize trigonometric functions. Any point on the unit circle can be represented by the coordinates (cos θ, sin θ), where θ is the angle measured counterclockwise from the positive x-axis.

This relationship is the foundation of trigonometry and is essential for understanding wave patterns.

[hr]

[b]Sine Function[/b]

The sine function (sin θ) represents the y-coordinate of a point on the unit circle at angle θ.

[i]Properties:[/i]
- Range: Always produces values between -1 and 1
- Period: Repeats every 2π radians (360 degrees)
- Shape: Smooth, periodic wave pattern

When we plot sin(θ) against θ, we get a smooth wave that is fundamental to understanding many natural phenomena, including sound waves and light.

[hr]

[b]Cosine Function[/b]

The cosine function (cos θ) represents the x-coordinate of a point on the unit circle at angle θ.

[i]Properties:[/i]
- Range: Always produces values between -1 and 1
- Period: Repeats every 2π radians (360 degrees)
- Phase: Same shape as sine, but shifted by π/2 radians (90 degrees)

This phase shift relationship between sine and cosine is crucial for understanding wave interactions.

[hr]

[b]Tangent Function[/b]

The tangent function (tan θ) is defined as the ratio of sine to cosine:

tan θ = sin θ / cos θ

Geometrically, it represents the slope of the line from the origin to the point on the unit circle.

[i]Properties:[/i]
- Range: From negative infinity to positive infinity
- Undefined when cos θ = 0 (at π/2, 3π/2, etc.)
- Period: Repeats every π radians (180 degrees)
- Has vertical asymptotes

[hr]

[b]Wave Combinations[/b]

Complex wave patterns can be created by combining sine and cosine waves with different:

- [i]Amplitudes:[/i] Controls the height of the wave
- [i]Frequencies:[/i] Controls how many cycles occur in a given distance
- [i]Phases:[/i] Controls the horizontal shift of the wave

Any periodic waveform can be represented as a sum of sine and cosine waves (Fourier series).

This principle is fundamental to understanding complex patterns in nature and is the basis for sound synthesis and analysis.
'''
