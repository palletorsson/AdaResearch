# Eye shot — Chamber_Color

> one pass: ride (gaze), move (place --only-improve), gate (pathfinder), voice (qfep). Field note, not a ruling.

## The ride (before)
clearance violations: **2 overlaps, 0 tight** — the law wants ≥1.2m to walk between.
- `[OVERLAP] proximity_spawner#type:kaleidocycle_enemy <-> catalyst_target            gap -0.38m (centers 4.12m)`
- `[OVERLAP] becoming_catalyst#start_mode:chromatic <-> catalyst_target            gap -0.89m (centers 3.61m)`

## The move
    logged 1 entries to ada_run\placement_log.json
    summary: 1 maps processed
      constraint improved: 0/1  mean Δ=+0.000
      walkability improved: 0/1  mean Δ=+0.000

no sibling kept — the move did not beat the ride (overlaps 2→2, tight 0→0). Note-only.

## The voice (qfep)
4 of 4 cast members carry a theory-claim; 0 mute.
- **becoming_catalyst** — Embodies the full QFEP trajectory: from order (primitives) through entropy (randomness) to the edge of chaos (
- **catalyst_target** — Transformation here is discrete state change: intact, flashing, destroyed, respawned. Identity persists while 
- **proximity_spawner** — E_entropy: the vent that answers approach — it spawns when the player comes near, so population is a function 
- **science_screen** — Transformation here is projection: a 3D state is re-expressed in 2D while preserving selected structural relat

## The text vs the space
walked.md exists — the writing names 4/4 of the cast; dwells declared for 1.
- **the writing's subjects are blocked in space**: becoming_catalyst, catalyst_target, proximity_spawner sit in clearance violations — the text promises what the floor obstructs.

## The heuristic understanding
The violations are real but mechanical moving does not fix them — they are placement DECISIONS (which body yields?), not placement errors. This is verdict material, not tooling material.
