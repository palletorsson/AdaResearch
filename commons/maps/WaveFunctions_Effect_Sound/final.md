...
Two balls hang above separate desks. Begin with one note.
...
Leave the blue ball where it is. Lift the orange one a little and listen again; PLAY sounds the current setting without another movement. While the ball is still moving the rig starts a fresh note every third of a second, so a drag is a run of beginnings; let the ball rest, and the note runs to its end. Before giving the change a name, decide where you heard it: at the beginning, through the middle, or near the end.
...
The scope offers short views of the modulator, carrier and output. The coloured cube gives the same settings another appearance. Neither can take over the listening.

In AirMusic, a strike starts a response that continues after contact. Carry this habit to the bar: give a sound time to change before deciding what sort of body has answered.
