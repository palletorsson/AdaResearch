...
## The Pendulum Is Not the Wave (2026-09-10)

The earlier notes concern prescribed sines — cubes that set their height from `sin(time)`. Their theoretical verdicts need to be read as proposals to test, not conclusions established by the presence of a sine function. The current primary, `control_pendulum`, advances an angle and angular velocity through successive physics steps. It uses the frame's interval, but it does not look up its state from the elapsed clock. Damping and the hand's release enter that history. Skipping the updates would change the result.

This gives the theoretical discussion a specific difference to work with. What survives a repetition? For the sine cube, an address in time is enough to recover a prescribed height. For this pendulum, the procedure carries state forward. That distinction can help us question accounts of repetition and performativity; it does not by itself demonstrate or refute Butler. Two releases from the same angle at different speeds make the narrower claim visible: position alone does not specify the next movement.

The room lets a hand enter this difference. While held, the bob follows the grip; on release, the algorithm retains an angle and a projected velocity. It discards the rest of the gesture. The crossing counter keeps another very thin past, a total without the circumstances of each pass. What can we learn from those reductions, and what would need another instrument to remain available?
