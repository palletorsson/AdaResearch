...
static func create(element_name: String, w: float, h: float, p: Dictionary = {}) -> Node3D:
	match element_name:
		...
		# Columns (Procedural mesh)
		"col_solomonic":
			return col_solomonic(w, h, p)
		"col_double_helix":
			return col_double_helix(w, h, p)
		"col_straight":
			return col_straight(w, h, p)
		"col_profile":
			return col_profile(w, h, p)
		"col_fluted_twist":
			return col_fluted_twist(w, h, p)
		"col_cluster":
			return col_cluster(w, h, p)
		# Openings
		"rect_window":
...
static func get_element_names() -> PackedStringArray:
	return PackedStringArray([
		...
		# Columns (Procedural mesh — ProceduralColumns)
		"col_solomonic", "col_double_helix", "col_straight",
		"col_profile", "col_fluted_twist", "col_cluster",
		# Openings
...
# ==========================================================================
# EXTENDED COLUMN TYPES (ProceduralColumns — SurfaceTool mesh geometry)
# ==========================================================================

## Solomonic (barley-twist) column — helical shaft spiraling like a corkscrew.
## Uses ProceduralColumns for SurfaceTool-based mesh generation.
static func col_solomonic(w: float, h: float, p: Dictionary = {}) -> Node3D:
	return ProceduralColumns.solomonic(w, h, p)


## Double helix column — two intertwined helical shafts (DNA / caduceus style).
static func col_double_helix(w: float, h: float, p: Dictionary = {}) -> Node3D:
	return ProceduralColumns.double_helix(w, h, p)


## Simple straight cylindrical column — clean, modern, no taper or flutes.
static func col_straight(w: float, h: float, p: Dictionary = {}) -> Node3D:
	return ProceduralColumns.straight(w, h, p)


## Profile/lathe column — any shape generated by revolving a 2D profile curve.
## Pass "profile" in params as a preset name or Array[Vector2].
static func col_profile(w: float, h: float, p: Dictionary = {}) -> Node3D:
	return ProceduralColumns.profile_column(w, h, p)


## Fluted twist column — solomonic shaft with spiral-following flute channels.
static func col_fluted_twist(w: float, h: float, p: Dictionary = {}) -> Node3D:
	return ProceduralColumns.fluted_twist(w, h, p)


## Gothic clustered column — central shaft with satellite shafts in a ring.
static func col_cluster(w: float, h: float, p: Dictionary = {}) -> Node3D:
	return ProceduralColumns.cluster(w, h, p)
