...
func _execute(mesh: MeshData, selected: PackedInt32Array) -> void:
	...
	var sentence := lsys.get_sentence()
	...
	for face_idx in selected:
		...
		var start_normal := mesh.get_face_normal(face_idx)
		var start_pos := mesh.get_face_centroid(face_idx)
		...
		for ci in range(sentence.length()):
			...
			match ch:
				"F", "S":
					...
					var end_pos := turtle_pos + turtle_dir * turtle_length
					var end_radius := turtle_radius * step_taper
				...
				"]":
					...
					if stack.size() > 0:
						...
						turtle_right = state["right"]
						turtle_up = state["up"]
						turtle_radius = state["radius"]
						turtle_length = state["length"]
...
func _add_tube_segment(mesh: MeshData, start: Vector3, end: Vector3,
		direction: Vector3, start_radius: float, end_radius: float,
	...
	for ri in range(rings):
		for si in range(segments):
			var si_next := (si + 1) % segments
			var tag := tip_tag_str if ri == rings - 1 else branch_tag
			mesh.add_face(
