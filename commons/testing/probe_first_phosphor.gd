## probe_first_phosphor.gd — do the three forces prop-gallery artifacts actually BUILD?
##
...
##
## Writes user://first_phosphor_probe.json and quits. The 4.6 exe is non-console, so the
...
##     <godot> --headless --path . --xr-mode off --script res://commons/testing/probe_first_phosphor.gd
extends SceneTree

const TOKENS := ["first_phosphor"]
...
func _run() -> void:
	for token in TOKENS:
		...
		await process_frame
		await process_frame
	var f := FileAccess.open("user://first_phosphor_probe.json", FileAccess.WRITE)
