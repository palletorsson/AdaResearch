## probe_zoetrope.gd — do the three forces prop-gallery artifacts actually BUILD?
##
...
##
## Writes user://zoetrope_probe.json and quits. The 4.6 exe is non-console, so the
...
##     <godot> --headless --path . --xr-mode off --script res://commons/testing/probe_zoetrope.gd
extends SceneTree

const TOKENS := ["grand_zoetrope"]
...
func _run() -> void:
	for token in TOKENS:
		...
		await process_frame
		await process_frame
	var f := FileAccess.open("user://zoetrope_probe.json", FileAccess.WRITE)
