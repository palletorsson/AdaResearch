...
func run() -> void:
	...
	if capture:
		...
		if board != null:
			...
			get_viewport().get_texture().get_image().save_png(OUT + "probe_wall_live.png")
			...
			if rn != null:
				...
				get_viewport().get_texture().get_image().save_png(OUT + "probe_wall_readout_live.png")
		...
		if room_body != null:
			...
			for i in range(14): cam.make_current(); await get_tree().process_frame
			await get_tree().create_timer(0.25, true, false, true).timeout
			get_viewport().get_texture().get_image().save_png(OUT + "probe_wall_interior_live.png")
		...
		for i in range(14): cam.make_current(); await get_tree().process_frame
		await get_tree().create_timer(0.25, true, false, true).timeout
		get_viewport().get_texture().get_image().save_png(OUT + "probe_wall_plan_live.png")
	...
	if _live():
		...
		if capture:
			...
			for i in range(8): await get_tree().process_frame
			await get_tree().create_timer(0.3, true, false, true).timeout
			get_viewport().get_texture().get_image().save_png(OUT + "probe_wall_desktop_front_live.png")
