## probe_choreography.gd — do the three forces prop-gallery artifacts actually BUILD?
##
...
##
## Writes user://choreography_probe.json and quits. The 4.6 exe is non-console, so the
...
##     <godot> --headless --path . --xr-mode off --script res://commons/testing/probe_choreography.gd
extends SceneTree

const TOKENS := ["the_choreography"]
...
func _run() -> void:
	for token in TOKENS:
		...
		await process_frame
		await process_frame
	var f := FileAccess.open("user://choreography_probe.json", FileAccess.WRITE)
