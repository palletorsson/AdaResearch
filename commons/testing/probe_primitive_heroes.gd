## probe_primitive_heroes.gd — do the three forces prop-gallery artifacts actually BUILD?
##
...
##
## Writes user://primitive_heroes_probe.json and quits. The 4.6 exe is non-console, so the
...
##     <godot> --headless --path . --xr-mode off --script res://commons/testing/probe_primitive_heroes.gd
extends SceneTree

const TOKENS := ["the_invisible_point", "first_shadow", "seven_words_choir", "backface_curtain", "budget_of_smoothness", "wireframe_confession"]
...
func _run() -> void:
	for token in TOKENS:
		...
		await process_frame
		await process_frame
	var f := FileAccess.open("user://primitive_heroes_probe.json", FileAccess.WRITE)
