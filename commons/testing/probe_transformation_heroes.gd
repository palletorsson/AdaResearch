## probe_transformation_heroes.gd — do the three forces prop-gallery artifacts actually BUILD?
##
...
##
## Writes user://transformation_heroes_probe.json and quits. The 4.6 exe is non-console, so the
...
##     <godot> --headless --path . --xr-mode off --script res://commons/testing/probe_transformation_heroes.gd
extends SceneTree

const TOKENS := ["baggage_grammar", "sleepwalking_bed", "furniture_turntable", "italic_bookcase", "two_cakes", "matrix_stamps", "identity_lineup"]
...
func _run() -> void:
	for token in TOKENS:
		...
		await process_frame
		await process_frame
	var f := FileAccess.open("user://transformation_heroes_probe.json", FileAccess.WRITE)
