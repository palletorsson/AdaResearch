## LIVE PORT — generated by tools/port_wcn_probes_live.py from probe_wcn_game.gd. Do not edit;
## edit the SceneTree probe and regenerate. Runs under project startup (probe_live.tscn
## with --probe=game), so the project's autoloads exist and XR Tools pickables compile.
...
func run() -> void:
	...
	if capture:
		...
		get_viewport().get_texture().get_image().save_png(_evidence + "probe_game_live.png")
		...
		await get_tree().create_timer(0.3, true, false, true).timeout
		get_viewport().get_texture().get_image().save_png(_evidence + "probe_game_approach_live.png")
		...
		if tnode != null:
			...
			for i in range(16): cam.make_current(); await get_tree().process_frame
			await get_tree().create_timer(0.3, true, false, true).timeout
			get_viewport().get_texture().get_image().save_png(_evidence + "probe_game_tablet_live.png")
		...
		for i in range(16): cam.make_current(); await get_tree().process_frame
		await get_tree().create_timer(0.3, true, false, true).timeout
		get_viewport().get_texture().get_image().save_png(_evidence + "probe_game_row_live.png")
		...
		if stele_node != null:
			...
			for i in range(16): cam.make_current(); await get_tree().process_frame
			await get_tree().create_timer(0.3, true, false, true).timeout
			get_viewport().get_texture().get_image().save_png(_evidence + "probe_game_stele_live.png")
		...
		for i in range(16): cam.make_current(); await get_tree().process_frame
		await get_tree().create_timer(0.3, true, false, true).timeout
		get_viewport().get_texture().get_image().save_png(_evidence + "probe_game_bed_live.png")
		...
		if idol != null:
			...
			for i in range(16): cam.make_current(); await get_tree().process_frame
			await get_tree().create_timer(0.3, true, false, true).timeout
			get_viewport().get_texture().get_image().save_png(_evidence + "probe_game_idol_live.png")
		...
		for i in range(16): cam.make_current(); await get_tree().process_frame
		await get_tree().create_timer(0.3, true, false, true).timeout
		get_viewport().get_texture().get_image().save_png(_evidence + "probe_game_plan_live.png")
	...
	if _live():
		...
		if capture:
			await get_tree().create_timer(0.3, true, false, true).timeout
			get_viewport().get_texture().get_image().save_png(_evidence + "probe_game_desktop_crossing_live.png")
		...
		if capture:
			await get_tree().create_timer(0.2, true, false, true).timeout
			get_viewport().get_texture().get_image().save_png(_evidence + "probe_game_desktop_fallen_live.png")
...
func _finish() -> void:
	...
	var f := FileAccess.open(OUT + "probe_game_live.json", FileAccess.WRITE)
