## probe_change_heroes.gd — do the three forces prop-gallery artifacts actually BUILD?
##
...
##
## Writes user://change_heroes_probe.json and quits. The 4.6 exe is non-console, so the
...
##     <godot> --headless --path . --xr-mode off --script res://commons/testing/probe_change_heroes.gd
extends SceneTree

const TOKENS := ["tangent_toboggan", "staircase_to_curve", "paper_regatta", "undo_door"]
...
func _run() -> void:
	for token in TOKENS:
		...
		await process_frame
		await process_frame
	var f := FileAccess.open("user://change_heroes_probe.json", FileAccess.WRITE)
