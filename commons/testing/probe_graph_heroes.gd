## probe_graph_heroes.gd — do the three forces prop-gallery artifacts actually BUILD?
##
...
##
## Writes user://graph_heroes_probe.json and quits. The 4.6 exe is non-console, so the
...
##     <godot> --headless --path . --xr-mode off --script res://commons/testing/probe_graph_heroes.gd
extends SceneTree

const TOKENS := ["family_chandelier", "postman_of_paths", "ouroboros_signals", "wallflower_and_star", "archipelago_wedding", "two_travelers", "impatient_river", "the_tree_you_are_in"]
...
func _run() -> void:
	for token in TOKENS:
		...
		await process_frame
		await process_frame
	var f := FileAccess.open("user://graph_heroes_probe.json", FileAccess.WRITE)
