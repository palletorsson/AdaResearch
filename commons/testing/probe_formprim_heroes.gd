## probe_formprim_heroes.gd — do the three forces prop-gallery artifacts actually BUILD?
##
...
##
## Writes user://formprim_heroes_probe.json and quits. The 4.6 exe is non-console, so the
...
##     <godot> --headless --path . --xr-mode off --script res://commons/testing/probe_formprim_heroes.gd
extends SceneTree

const TOKENS := ["laundry_line_cathedral", "impossible_end_table", "cookie_press"]
...
func _run() -> void:
	for token in TOKENS:
		...
		await process_frame
		await process_frame
	var f := FileAccess.open("user://formprim_heroes_probe.json", FileAccess.WRITE)
