## probe_two_costumes.gd — do the three forces prop-gallery artifacts actually BUILD?
##
...
##
## Writes user://two_costumes_probe.json and quits. The 4.6 exe is non-console, so the
...
##     <godot> --headless --path . --xr-mode off --script res://commons/testing/probe_two_costumes.gd
extends SceneTree

const TOKENS := ["two_costumes"]
...
func _run() -> void:
	for token in TOKENS:
		...
		await process_frame
		await process_frame
	var f := FileAccess.open("user://two_costumes_probe.json", FileAccess.WRITE)
