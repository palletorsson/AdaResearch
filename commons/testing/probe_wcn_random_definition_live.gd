## LIVE PORT — generated by tools/port_wcn_probes_live.py from probe_wcn_random_definition.gd. Do not edit;
## edit the SceneTree probe and regenerate. Runs under project startup (probe_live.tscn
## with --probe=random_definition), so the project's autoloads exist and XR Tools pickables compile.
...
func run() -> void:
	...
	if _live():
		...
		if "--capture" in OS.get_cmdline_user_args():
			...
			get_viewport().get_texture().get_image().save_png(_evidence + "probe_random_definition_desktop_close_live.png")
			...
			await get_tree().create_timer(0.3, true, false, true).timeout
			get_viewport().get_texture().get_image().save_png(_evidence + "probe_random_definition_desktop_approach_live.png")
	...
	if "--capture" in OS.get_cmdline_user_args():
		...
		await get_tree().create_timer(0.3, true, false, true).timeout
		get_viewport().get_texture().get_image().save_png(_evidence + "probe_random_definition_live.png")
	_finish()
...
func _finish() -> void:
	...
	var f := FileAccess.open(OUT + "probe_random_definition_live.json", FileAccess.WRITE)
