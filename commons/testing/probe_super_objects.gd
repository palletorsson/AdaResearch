## probe_super_objects.gd — do the three forces prop-gallery artifacts actually BUILD?
##
...
##
## Writes user://super_objects_probe.json and quits. The 4.6 exe is non-console, so the
...
##     <godot> --headless --path . --xr-mode off --script res://commons/testing/probe_super_objects.gd
extends SceneTree

const TOKENS := ["orrery_of_forces", "chromatic_organ", "switchboard_constellation", "genesis_lathe"]
...
func _run() -> void:
	for token in TOKENS:
		...
		await process_frame
		await process_frame
	var f := FileAccess.open("user://super_objects_probe.json", FileAccess.WRITE)
