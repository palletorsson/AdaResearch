## probe_casino.gd — do the three forces prop-gallery artifacts actually BUILD?
##
...
##
## Writes user://casino_probe.json and quits. The 4.6 exe is non-console, so the
...
##     <godot> --headless --path . --xr-mode off --script res://commons/testing/probe_casino.gd
extends SceneTree

const TOKENS := ["replay_casino"]
...
func _run() -> void:
	for token in TOKENS:
		...
		await process_frame
		await process_frame
	var f := FileAccess.open("user://casino_probe.json", FileAccess.WRITE)
