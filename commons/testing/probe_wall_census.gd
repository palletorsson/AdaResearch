...
func _run() -> void:
	...
	var lines: Array = []
	for t in talls.slice(0, 14):
		lines.append("%.1f m top · %s '%s' under '%s'" % [t["top"], t["cls"], t["name"], t["parent"]])
...
func _walk(n: Node, out_list: Array) -> void:
	if n is MeshInstance3D and (n as MeshInstance3D).mesh != null:
		...
		if top > 1.5:
			out_list.append({"top": top, "cls": "MeshInstance3D", "name": n.name, "parent": n.get_parent().name})
