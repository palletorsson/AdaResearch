...
func run() -> void:
	...
	if _live():
		...
		if "--capture" in OS.get_cmdline_user_args():
			...
			root.get_texture().get_image().save_png(_evidence + "probe_pendulum_desktop_walk.png")
	...
	if "--capture" in OS.get_cmdline_user_args():
		...
		await create_timer(0.3, true, false, true).timeout
		root.get_texture().get_image().save_png(_evidence + "probe_pendulum.png")
		...
		if readout != null:
			...
			await create_timer(0.3, true, false, true).timeout
			root.get_texture().get_image().save_png(_evidence + "probe_pendulum_readout.png")
		...
		if panel != null:
			...
			for i in range(20): cam.make_current(); await process_frame
			await create_timer(0.3, true, false, true).timeout
			root.get_texture().get_image().save_png(_evidence + "probe_pendulum_controls.png")
