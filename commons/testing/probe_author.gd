## probe_author.gd — do the three forces prop-gallery artifacts actually BUILD?
##
...
##
## Writes user://author_probe.json and quits. The 4.6 exe is non-console, so the
...
##     <godot> --headless --path . --xr-mode off --script res://commons/testing/probe_author.gd
extends SceneTree

const TOKENS := ["the_absent_author"]
...
func _run() -> void:
	for token in TOKENS:
		...
		await process_frame
		await process_frame
	var f := FileAccess.open("user://author_probe.json", FileAccess.WRITE)
