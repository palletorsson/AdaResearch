## probe_confessing.gd — do the three forces prop-gallery artifacts actually BUILD?
##
...
##
## Writes user://confessing_probe.json and quits. The 4.6 exe is non-console, so the
...
##     <godot> --headless --path . --xr-mode off --script res://commons/testing/probe_confessing.gd
extends SceneTree

const TOKENS := ["the_confessing_body"]
...
func _run() -> void:
	for token in TOKENS:
		...
		await process_frame
		await process_frame
	var f := FileAccess.open("user://confessing_probe.json", FileAccess.WRITE)
