## probe_prop_gallery.gd — do the three forces prop-gallery artifacts actually BUILD?
##
...
##
## Writes user://prop_gallery_probe.json and quits. The 4.6 exe is non-console, so the
...
const TOKENS := ["paused_fountain", "prop_mobile", "prop_spigot", "revolving_door", "three_gravities", "parlour_orbits", "umbrella_field", "momentum_cradle", "brake_skid", "drag_corridor", "spring_tower", "tug_of_war", "matryoshka_row", "sisyphus_mower", "prop_carousel", "invisible_hill", "origin_doormat"]
...
func _run() -> void:
	for token in TOKENS:
		...
		await process_frame
		await process_frame
	var f := FileAccess.open("user://prop_gallery_probe.json", FileAccess.WRITE)
