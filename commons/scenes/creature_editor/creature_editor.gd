...
func _build_kingdom_buttons() -> void:
	...
	for kname in ["Tree", "Creature", "Flower", "Fungus", "Hybrid"]:
		var btn := Button.new()
		btn.text = kname
...
func _build_preset_buttons() -> void:
	...
	grid.columns = 3
	var preset_names := ["lsystem_tree", "fibonacci_flower", "turing_fungus",
...
func _build_gene_sliders() -> void:
	...
	for group in GENE_GROUPS:
		...
		for gene in group["genes"]:
			...
			slider.min_value = gene["min"]
			slider.max_value = gene["max"]
			slider.step = gene["step"]
			slider.size_flags_horizontal = Control.SIZE_EXPAND_FILL
			slider.value_changed.connect(_on_gene_changed.bind(gene["id"]))
...
func _load_preset(preset_name: String) -> void:
	...
	if placer_script and preset_name in placer_script.PRESETS:
		var preset: Dictionary = placer_script.PRESETS[preset_name]
		_dna = CritterDNA.new()
		var kingdom: int = preset.get("kingdom", 0)
		_dna.body_type = float(kingdom) if kingdom >= 0 else preset.get("body_type", 1.0)
		...
		if preset.has("secondary_color"):
			_dna.secondary_color = Color(preset["secondary_color"])
	else:
		_dna = CritterDNA.random()
	_spawn_from_dna()
...
func _clear_organism() -> void:
	...
	for child in _organism_root.get_children():
		child.queue_free()
	_spawner._active_critters.clear()
