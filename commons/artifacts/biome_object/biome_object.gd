	...
	for key in _cells.keys():
		...
		for i in range(shards):
			...
			if pieces <= 1:
				...
				mi.mesh = pm
				# the ring the pieces leave with `sample`: the SMALLEST one on which pieces of this
				# width stand clear of each other, r = w / (2 sin(pi/n)), never under the drawn spread.
				# Scaling the ring by a_sub instead (the first try) collapsed the halves of a
				# half-split stone into one another, and the morph read as one solid until the last
				# frame — a split nobody could see is not a split.
				var ang: float = TAU * float(i) / float(pieces)
				var rr: float = maxf(0.12 * scale, pm.size.x / (2.0 * sin(PI / float(pieces))))
				var reg := Vector2(cos(ang), sin(ang)) * rr
				var o: Vector2 = off if (a_sample >= 1.0 and a_sub >= 1.0) else Vector2(_mix(reg.x, off.x, a_sample), _mix(reg.y, off.y, a_sample))
