...
# @identity
# essence: a heap of offcuts on one side of the room and a bare table on the other — twelve mismatched fragments nobody designed together, each one liftable, and a surface that quietly squares whatever you set down on it
...
## deterministic scatter of primitive offcuts, each a real XRToolsPickable body.
## Right: a bare assembly table with a coarse grid scribed into it. Fragments
## dropped over the table snap to that grid and stay put.
...
# --- knobs ----------------------------------------------------------------
## Fragments dealt onto the heap. Mismatched by construction — the shape, the
## proportion and the colour are drawn independently, so no two agree.
@export var fragment_count: int = 12
## Cell size of the assembly table's grid, in metres.
@export var snap_step: float = 0.14
...
func _build_pile() -> void:
	...
	for i in 3:
		...
		var bm := BoxMesh.new()
		bm.size = Vector3(0.62, 0.05, 0.22)
		slab.mesh = bm
		var ang: float = TAU * float(i) / 3.0 + 0.6
		slab.position = PILE_C + Vector3(cos(ang) * 0.5, PILE_TOP - 0.03, sin(ang) * 0.5)
...
## The snap grid, drawn as hairlines flush with the table top. This is the only
## instruction the table gives, and it gives it before anything is placed.
func _scribe_grid() -> void:
	...
	for i in range(-nx, nx + 1):
		...
		var lm := BoxMesh.new()
		lm.size = Vector3(0.006, 0.004, TABLE_HZ * 1.9)
		line.mesh = lm
		line.position = TABLE_C + Vector3(float(i) * step, TABLE_TOP + 0.002, 0.0)
	...
	for j in range(-nz, nz + 1):
		...
		var lm2 := BoxMesh.new()
		lm2.size = Vector3(TABLE_HX * 1.9, 0.004, 0.006)
		line2.mesh = lm2
		line2.position = TABLE_C + Vector3(0.0, TABLE_TOP + 0.002, float(j) * step)
...
func _build_plate() -> void:
	...
	ts.width_m = 0.40
	ts.position = TABLE_C + Vector3(0.0, TABLE_TOP + 0.006, TABLE_HZ - 0.09)
...
## pile is byte-identical on every load — the scatter is authored, not rolled.
func _deal_fragments() -> void:
	var host: Node = _fragment_host()
	var n: int = clampi(fragment_count, 4, 24)
	for i in n:
		...
		# Seated on the heap, frozen: no settling drift, so the arrangement you
		# see is exactly the arrangement the seed describes.
		var ang: float = _rng.randf_range(0.0, TAU)
		var rad: float = sqrt(_rng.randf()) * (PILE_R - 0.16)
		var local := PILE_C + Vector3(cos(ang) * rad, PILE_TOP + half_h + _rng.randf_range(0.0, 0.05), sin(ang) * rad)
...
## would list — the point is that they were made for something else.
func _fragment_mesh(kind: String) -> Mesh:
	match kind:
		"slab":
			var slab := BoxMesh.new()
			slab.size = Vector3(_rng.randf_range(0.16, 0.26), 0.035, _rng.randf_range(0.09, 0.16))
			return slab
		"rod":
			var rod := CylinderMesh.new()
			rod.top_radius = _rng.randf_range(0.022, 0.04)
			rod.bottom_radius = rod.top_radius
			rod.height = _rng.randf_range(0.14, 0.24)
			rod.radial_segments = 8
			return rod
		"disc":
			var disc := CylinderMesh.new()
			disc.top_radius = _rng.randf_range(0.07, 0.11)
			disc.bottom_radius = disc.top_radius
			disc.height = _rng.randf_range(0.02, 0.04)
			disc.radial_segments = 16
			return disc
		"ring":
			var ring := TorusMesh.new()
			ring.inner_radius = _rng.randf_range(0.045, 0.07)
			ring.outer_radius = ring.inner_radius + _rng.randf_range(0.02, 0.035)
			ring.rings = 14
			ring.ring_segments = 8
			return ring
		"wedge":
			var wedge := PrismMesh.new()
			wedge.size = Vector3(_rng.randf_range(0.11, 0.18), _rng.randf_range(0.08, 0.14), 0.09)
			return wedge
	# "box" and anything unrecognised: an offcut cube with no two sides equal.
	var box := BoxMesh.new()
	box.size = Vector3(
		_rng.randf_range(0.07, 0.13),
		_rng.randf_range(0.07, 0.13),
		_rng.randf_range(0.07, 0.13))
	return box
...
func apply_grid_config(config_data: Dictionary) -> void:
	...
	if config_data.has("fragment_count"):
		fragment_count = clampi(int(config_data["fragment_count"]), 4, 24)
