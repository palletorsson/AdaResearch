...
class_name Rule110Bench

## @identity
## lineage: Rule 110 — proven Turing-complete by Cook/Wolfram; the elementary CA that is neither orderly nor random.
## essence: On a bench, a noisy row settles into a background weave crossed by gliders — particles that persist and collide.
## truth: Between dead crystal and pure noise lives computation. Four cells, one rule, and the edge is alive enough to run anything.
...
var _rows: Array = []
var _alive_col: Color = Color(0.9, 0.7, 1.0, 1)
...
func _build() -> void:
	...
	add_child(_billboard_label("RULE 110\nfour cells compute anything", Vector3(0, 1.6, 0), 28, _alive_col))
...
func _seed() -> void:
	# Rule 110 needs a busy initial condition to show gliders + a structured background.
	...
	# guarantee a single live cell on the right edge — the canonical Rule 110 seed grows a triangular wake
	first[cols - 1] = 1
	_rows.append(first)
