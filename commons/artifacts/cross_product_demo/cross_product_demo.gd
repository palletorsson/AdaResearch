...
## Display settings
@export var max_vector_length: float = 0.8
@export var arrow_thickness: float = 0.02

## Vector A
@export var vector_a: Vector3 = Vector3(0.5, 0.0, 0.0):
	set(value):
		vector_a = value.limit_length(max_vector_length)
		_update_visualization()

## Vector B
@export var vector_b: Vector3 = Vector3(0.0, 0.0, 0.5):
	set(value):
...
func _create_vr_controls():
	...
	for i in range(presets.size()):
		...
		btn.name = "Preset%d" % i
		btn.position = Vector3(-0.16 + i * 0.08, 0, -0.005)
