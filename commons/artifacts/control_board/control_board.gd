...
# ── Palette ──────────────────────────────────────────────────────────
const COL_DESK      := Color(0.12, 0.12, 0.14)
...
# ── Config ───────────────────────────────────────────────────────────
@export var desk_width: float = 2.4
@export var desk_depth: float = 0.8
@export var desk_height: float = 0.85
@export var panel_tilt: float = 25.0  # degrees

# ── State ────────────────────────────────────────────────────────────
var _crt_screens: Array[MeshInstance3D] = []
var _led_indicators: Array[Dictionary] = []  # {mesh, color_on, color_off, state}
...
func _ready() -> void:
	...
	_build_speaker()
	_build_led_strip()
	print("[ControlBoard] Built — %d CRTs, %d LEDs, %d faders" % [
...
func apply_grid_config(config_data: Dictionary) -> void:
	if config_data.has("desk_width"):
		desk_width = float(config_data["desk_width"])
	if config_data.has("desk_depth"):
		desk_depth = float(config_data["desk_depth"])
...
func _build_desk() -> void:
	# Main desktop surface (angled)
	var surface := MeshInstance3D.new()
	var mesh := BoxMesh.new()
	mesh.size = Vector3(desk_width, 0.04, desk_depth)
	surface.mesh = mesh
	surface.material_override = _mat(COL_DESK, 0.4, 0.6)
	surface.position = Vector3(0, desk_height, 0)
	surface.rotation_degrees.x = -panel_tilt
	add_child(surface)

	# Front panel face
	var front := MeshInstance3D.new()
	var fmesh := BoxMesh.new()
	fmesh.size = Vector3(desk_width, desk_height, 0.04)
	front.mesh = fmesh
	front.material_override = _mat(COL_PANEL, 0.3, 0.7)
	front.position = Vector3(0, desk_height * 0.5, desk_depth * 0.4)
	add_child(front)
	...
	for side in [-1.0, 1.0]:
		...
		var sm := BoxMesh.new()
		sm.size = Vector3(0.03, desk_height * 0.6, desk_depth * 0.6)
		sp.mesh = sm
		sp.material_override = _mat(COL_DESK, 0.4, 0.6)
		sp.position = Vector3(side * desk_width * 0.5, desk_height * 0.65, 0)
		add_child(sp)

# ══════════════════════════════════════════════════════════════════════
#  MONITORS — CRT screens with animated phosphor scanlines
# ══════════════════════════════════════════════════════════════════════

func _build_monitors() -> void:
	var panel_y := desk_height + 0.02
	var panel_z := -0.05
	var tilt := -panel_tilt

	# Large CRT left
	_add_crt(Vector3(-0.65, panel_y + 0.22, panel_z), Vector3(0.50, 0.38, 0.08), tilt)
	# Large CRT right
	_add_crt(Vector3(0.10, panel_y + 0.22, panel_z), Vector3(0.50, 0.38, 0.08), tilt)
	# Small CRT far right — waveform scope
	_add_crt(Vector3(0.72, panel_y + 0.20, panel_z), Vector3(0.30, 0.25, 0.06), tilt)

func _add_crt(pos: Vector3, size: Vector3, tilt_deg: float) -> void:
	var housing := MeshInstance3D.new()
	var hmesh := BoxMesh.new()
	hmesh.size = size + Vector3(0.04, 0.04, 0.02)
	housing.mesh = hmesh
	housing.material_override = _mat(COL_PANEL, 0.3, 0.8)
	housing.position = pos
	housing.rotation_degrees.x = tilt_deg
	add_child(housing)
	...
	screen.rotation_degrees.x = tilt_deg
	add_child(screen)
	_crt_screens.append(screen)

# ══════════════════════════════════════════════════════════════════════
#  METERS — VU meters with animated needles
# ══════════════════════════════════════════════════════════════════════

func _build_meters() -> void:
	var base_y := desk_height + 0.05
	var base_z := 0.08
	# Two VU meters side by side
	for i in 2:
		var x_pos: float = -0.95 + i * 0.22
		_add_vu_meter(Vector3(x_pos, base_y, base_z))
...
func _add_vu_meter(pos: Vector3) -> void:
	...
	face.position = pos
	face.rotation_degrees.x = -panel_tilt
	add_child(face)
	...
	needle.rotation_degrees.x = -panel_tilt
	add_child(needle)
	_vu_needles.append(needle)

# ══════════════════════════════════════════════════════════════════════
#  FADER BANK — 6 vertical faders using VR sliders
# ══════════════════════════════════════════════════════════════════════

func _build_fader_bank() -> void:
	var base_pos := Vector3(-0.40, desk_height + 0.02, 0.15)
	for i in 6:
		var slider := SLIDER_H.instantiate()
		slider.name = "Fader%d" % i
		slider.position = base_pos + Vector3(i * 0.1, 0, 0)
		slider.rotation_degrees.x = -panel_tilt
		slider.rotation_degrees.z = 90  # vertical orientation
...
func _on_fader_moved(_pos: Variant, index: int) -> void:
	if index < _faders.size() and _faders[index].has_method("get_normalized_value"):
		...
		# Drive VU needle for corresponding channel
		if index < _vu_needles.size():
			_vu_needles[index].rotation_degrees.z = lerp(-40.0, 40.0, val)

# ══════════════════════════════════════════════════════════════════════
#  BUTTON ARRAY — colored push buttons (interactive)
# ══════════════════════════════════════════════════════════════════════

func _build_button_array() -> void:
	var base := Vector3(0.55, desk_height + 0.02, 0.22)
	var button_defs := [
	...
	for i in button_defs.size():
		...
		btn.position = base + Vector3(col * 0.08, -row * 0.06, row * 0.04)
		btn.rotation_degrees.x = -panel_tilt
...
# ══════════════════════════════════════════════════════════════════════
#  SWITCH ROW — toggle switches (visual, clickable)
# ══════════════════════════════════════════════════════════════════════

func _build_switch_row() -> void:
	var base := Vector3(-0.90, desk_height + 0.01, 0.24)
	for i in 8:
		var sw_base := MeshInstance3D.new()
		var bm := BoxMesh.new()
		bm.size = Vector3(0.025, 0.015, 0.025)
		sw_base.mesh = bm
		sw_base.material_override = _mat(COL_METAL, 0.7, 0.3)
		sw_base.position = base + Vector3(i * 0.055, 0, 0)
		sw_base.rotation_degrees.x = -panel_tilt
		add_child(sw_base)
		...
		var up := (i % 2 == 0)
		lever.position = sw_base.position + Vector3(0, 0.02 if up else 0.008, -0.005 if up else 0.005)
		lever.rotation_degrees.x = (-panel_tilt + (-15 if up else 15))
		add_child(lever)

# ══════════════════════════════════════════════════════════════════════
#  KNOB CLUSTER — rotary dials
# ══════════════════════════════════════════════════════════════════════

func _build_knob_cluster() -> void:
	var base := Vector3(0.85, desk_height + 0.02, 0.12)
	var labels := ["FREQ", "GAIN", "BIAS", "TRIM"]
	for i in labels.size():
		var row: int = i / 2
		var col: int = i % 2
		var pos := base + Vector3(col * 0.12, -row * 0.06, row * 0.04)
...
func _add_knob(pos: Vector3, label_text: String, radius: float) -> void:
	...
	knob.position = pos
	knob.rotation_degrees.x = -panel_tilt + 90
	add_child(knob)

	# Pointer line
	var ptr := MeshInstance3D.new()
	var pm := BoxMesh.new()
	pm.size = Vector3(0.002, 0.008, radius * 0.8)
	ptr.mesh = pm
	ptr.material_override = _mat(Color.WHITE, 0.0, 0.5)
	ptr.position = pos + Vector3(0, 0.001, -0.01)
	ptr.rotation_degrees.x = -panel_tilt
	# Random initial rotation to look "set"
	ptr.rotation_degrees.y = randf_range(-120, 120)
	add_child(ptr)
	...
	lbl.position = pos + Vector3(0, -0.025, 0.01)
	lbl.rotation_degrees.x = -panel_tilt
	add_child(lbl)
...
func _build_speaker() -> void:
	...
	housing.position = Vector3(0.90, desk_height + 0.30, -0.15)
	add_child(housing)
...
func _build_led_strip() -> void:
	var base := Vector3(-0.50, desk_height + 0.01, 0.30)
	var led_colors := [
	...
	for i in led_colors.size():
		...
		var initially_on: bool = (i % 3 != 2)  # most start on
		...
		led.rotation_degrees.x = -panel_tilt
		add_child(led)
...
func _animate_leds(_delta: float) -> void:
	# Occasional random blink on some LEDs
	for i in _led_indicators.size():
		var led: Dictionary = _led_indicators[i]
		# Random blink every ~2 seconds for "activity" LEDs
