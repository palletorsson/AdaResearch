...
@export var stiffness: float = 0.24               # low = squishy dough
...
func _build() -> void:
	...
	# The held dough ball, centred at hand height (~0.5 m). No bench — this is a SMALL held toy.
	...
	# Anchor the BACK patch (−Z, at mid-height) to the world so the ball floats at hand height and
	# stays roughly spherical — pinning the bottom would let the soft mass droop down through the
	# floor. The front and underside stay free and pokable.
