# GridSystemEnhanced.gd
# Enhanced grid system that supports JSON map format
# Unified interface for loading and managing map data from JSON files

extends Node3D
class_name GridSystemEnhanced
...
func _ready():
	...
	rng.randomize()
	print("GridSystemEnhanced: Initializing...")
	...
	if use_enhanced_utility_handler:
		...
		print("GridSystemEnhanced: Enhanced utility handler initialized")
...
func _load_json_map() -> void:
	...
	print("GridSystemEnhanced: Loading JSON map from '%s'" % json_path)
	...
	if json_loader.load_map(json_path):
		...
		print("GridSystemEnhanced: JSON map loaded successfully")
...
func _finalize_map_loading() -> void:
	...
	if algorithm_registry.get_all_algorithm_ids().size() > 0:
		...
		print("GridSystemEnhanced: Algorithm registry empty, proceeding with grid generation...")
		_on_algorithm_registry_loaded()

func _on_algorithm_registry_loaded():
	print("GridSystemEnhanced: Algorithm registry loaded, generating grid...")
	base_cube.visible = false
	_generate_grid()

func _generate_grid() -> void:
	print("GridSystemEnhanced: Starting grid generation")
	_initialize_grid()
...
func _initialize_grid() -> void:
	print("GridSystemEnhanced: Initializing grid: %dx%dx%d" % [grid_x, grid_y, grid_z])
	grid = []
	cube_map.clear()
...
func _apply_structure_data() -> void:
	...
	print("GridSystemEnhanced: Applying structure data")
	...
	print("GridSystemEnhanced: Added %d cubes" % cube_count)
...
func _apply_utility_data() -> void:
	...
	print("GridSystemEnhanced: Applying utility data")
	var total_size = cube_size + gutter
	var utility_count = 0
	...
	if current_map_format == "json" and use_enhanced_utility_handler and enhanced_utility_handler:
		print("GridSystemEnhanced: Using enhanced utility handler for JSON format")
		...
		print("GridSystemEnhanced: Added %d utilities via enhanced handler" % utility_count)
		return
	
	# Fallback to original utility placement for legacy format or when enhanced handler is disabled
	print("GridSystemEnhanced: Using legacy utility placement")
	var utility_layout = utility_data_instance.layout_data
	...
	print("GridSystemEnhanced: Added %d utilities" % utility_count)
...
func _apply_interactable_data() -> void:
	...
	print("GridSystemEnhanced: Applying interactable data")
...
func _reload_current_map() -> void:
	print("GridSystemEnhanced: Reloading map '%s'" % map_name)
...
func _clear_all_objects() -> void:
	print("GridSystemEnhanced: Clearing all objects...")
	...
	print("GridSystemEnhanced: All objects cleared")
...
func _stop_all_audio() -> void:
	print("GridSystemEnhanced: Stopping all audio...")
...
func enable_enhanced_utility_handler():
	...
	if not enhanced_utility_handler:
		enhanced_utility_handler = EnhancedUtilityHandler.new(self)
		add_child(enhanced_utility_handler)
		print("GridSystemEnhanced: Enhanced utility handler enabled")
...
func disable_enhanced_utility_handler():
	...
	if enhanced_utility_handler:
		...
		print("GridSystemEnhanced: Enhanced utility handler disabled")
...
func set_utility_handler_type(handler_type: String):
	match handler_type.to_lower():
		...
		_:
			print("GridSystemEnhanced: Unknown utility handler type: %s" % handler_type) 
