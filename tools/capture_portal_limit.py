"""Capture the production portal polygon diagram in a private, minimal Godot project."""
from pathlib import Path
import hashlib, json, os, re, shutil, subprocess, tempfile
ROOT = Path(__file__).resolve().parents[1]
OUT = ROOT / 'doc/book/iterations/2026-09-23-portals-visible-limit/figures'
FILES = [
    'commons/ui/limit_polygon_diagram.gd',
    'commons/font/static/Roboto-Regular.ttf',
    'commons/font/static/Roboto-Light.ttf',
    'commons/font/JetBrainsMono-Medium.ttf',
    'tools/captures/portal_limit_diagram.gd',
]

def main():
    OUT.mkdir(parents=True, exist_ok=True)
    workspace = Path(tempfile.mkdtemp(prefix='ada-portal-diagram-'))
    hashes = {}
    for name in FILES:
        dest = workspace / name
    ...
    result = subprocess.run([str(godot), '--path', str(workspace), '--rendering-method', 'gl_compatibility', '--resolution', '128x128', '--position', '-10000,-10000', '--script', 'res://tools/captures/portal_limit_diagram.gd', '--', OUT.as_posix()], timeout=60, **common)
    log=result.stdout+result.stderr
    (OUT/'capture.log').write_text(log, encoding='utf-8')
    print(log[-3500:])
    (OUT/'provenance.json').write_text(json.dumps({'source_files':hashes,'godot':str(godot),'isolated_project':str(workspace),'same_scene_as_museum':True,'book_arrangement':'Three instances of the live museum diagram'},indent=2)+'\n', encoding='utf-8')
