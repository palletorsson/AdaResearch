#!/usr/bin/env python3
"""build_wave14_gallery.py — wave 14, six syntheses built by six agents under one orchestrator.
...
S = json.loads((REPO / "ada_run" / "wave14_scores.json").read_text(encoding="utf-8"))
...
from wave14_content import TITLE, MAST, SECTIONS, closing  # noqa: E402
CLOSING = closing(S)
...
def main() -> int:
    ...
    head = base[:cut].replace("<title>Wave 12 &mdash; the tip that never moves</title>",
                              "<title>Wave 14 &mdash; " + TITLE + "</title>")
    head += """<style>
...
"""
    body += CLOSING
    out = ENC / "public/galleries/wave-14.html"
