"""Publishable review from a passing run whose complete source hashes still match."""
from pathlib import Path
import hashlib,json,re,shutil
import markdown
R=Path(__file__).resolve().parents[1];O=R/'doc/research/possible-bodies';E=R/'doc/space/genetic-evolution-2026-09-14';RUN=R/'ada_run/genetic-review-2026-09-14';M='PG_Genetic_Evolution'
...
assets=[]
for name in ['entrance','target','uncounted-ring','generation','controls','platforms','ramp']:
 p=O/f'genetic-{name}.png';shutil.copyfile(RUN/M/f'{name}.png',p);assets.append(p.name)
for kind in ['final','technical','critical','tutorial']:
 for previous in [False,True]:
  if previous and not (E/'before'/f'commons/maps/{M}/{kind}.md').exists():continue
  p=O/f'genetic-{"previous-" if previous else ""}{kind}.md';shutil.copyfile((E/'before' if previous else R)/f'commons/maps/{M}/{kind}.md',p);assets.append(p.name)
dump(O/'genetic-verification.json',r);assets.append('genetic-verification.json')
...
page=(R/'ada_run/genetic-page.html').read_text()
(O/'genetic-evolution.html').write_bytes(page.replace('__CSS__',css).replace('__BOOK__',book).replace('__CHECKS__',str(r['checks'])).encode());assets.append('genetic-evolution.html')
method=markdown.markdown((O/'spine-iteration.md').read_text(),extensions=['tables','toc'])
(O/'spine-iteration.html').write_bytes(('<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>First traverse. Then return. · Ada Research</title><style>'+css+'table{width:100%;border-collapse:collapse;font-size:.9rem}td,th{border-bottom:1px solid #425050;padding:.6rem;text-align:left;vertical-align:top}@media(max-width:650px){table{display:block;overflow-x:auto}}</style></head><body><main><nav><a href="genetic-evolution.html">Genetic Evolution</a><a href="pg-floor-audit.html">Floor audit</a><a href="/museum-progress">Museum progress</a></nav><article class="manuscript">'+method+'</article></main></body></html>').encode())
...
print(f'Built Genetic Evolution review: {len(targets)} assets; {r["checks"]} runtime checks.')
