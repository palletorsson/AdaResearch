#!/usr/bin/env python
"""
primitive_stack_research.py — Batch-render primitive_stack configs.
Mirror of graph_grammar_research.py but for primitive compositions.
"""
...
CONFIG_PATH = REPO_ROOT / "commons" / "primitive_grammar" / "research_configs.json"
ENCYCLOPEDIA_DIR = REPO_ROOT.parent / "ada_encyclopedia"
OUTPUT_DIR = ENCYCLOPEDIA_DIR / "public" / "primitive-stack-gallery"
STAGE_DIR_NAME = "ps_gallery"
...
def render_one(config: dict, force: bool) -> bool:
    ...
    staging = REPO_ROOT / "commons" / "primitive_grammar" / "_staging"
    ...
    print(f"    -> public/primitive-stack-gallery/{cid}.png")
    return True
...
def main() -> int:
    ...
    print(f"Primitive stack: {len(configs)} configs -> {OUTPUT_DIR}")
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    ok, fail = [], []
    for cfg in configs:
    ...
    for cfg in lib["configs"]:
        ...
        entries.append({
            ...
            "image": f"/primitive-stack-gallery/{cfg['id']}.png",
            "config": f"/primitive-stack-gallery/{cfg['id']}.json",
        })
    manifest = {
