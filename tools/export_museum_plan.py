...
def plan_museum(key: str, tokens: list[str],
    ...
    if cell_overrides:
        from spatial_floorplan import _apply_cell_overrides
        plan = _apply_cell_overrides(plan, cell_overrides, APRON)   # the hand's masonry binds the negotiation
    ...
    for p in placements:
        ...
        placed.append({
            ...
            **({"fill": True} if (p.artifact in fill_set or (saturate and p.artifact == saturate)) else {}),
            **({"wall_fit": dict(p.wall_fit)} if getattr(p, "wall_fit", None) else {}),
            **({"dummy": True} if (saturate and p.artifact == saturate) else {}),
