#!/usr/bin/env python
"""
rd_research.py — Batch-render Reaction-Diffusion configs.
DNA = preset or (F, K) + iterations + grid_size + seed + render_mode.
"""
...
CONFIG_PATH = REPO_ROOT / "commons" / "rd_grammar" / "research_configs.json"
ENCYCLOPEDIA_DIR = REPO_ROOT.parent / "ada_encyclopedia"
OUTPUT_DIR = ENCYCLOPEDIA_DIR / "public" / "rd-gallery"
STAGE_DIR_NAME = "rd_gallery"
...
def render_one(config: dict, force: bool) -> bool:
    ...
    staging = REPO_ROOT / "commons" / "rd_grammar" / "_staging"
    ...
    res_cfg = f"res://commons/rd_grammar/_staging/{cid}.json"
    args = [
    ...
    print(f"  render {cid} ({config.get('render_mode','heightmap')}) ...")
    ...
    print(f"    -> public/rd-gallery/{cid}.png")
    return True
...
def main() -> int:
    ...
    print(f"RD: {len(configs)} configs -> {OUTPUT_DIR}")
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    ok, fail = [], []
    for cfg in configs:
    ...
    for cfg in lib["configs"]:
        ...
        entries.append({
            ...
            "image": f"/rd-gallery/{cfg['id']}.png",
            "config": f"/rd-gallery/{cfg['id']}.json",
        })
    manifest = {
