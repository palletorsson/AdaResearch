...
def derive_row(pearl: str, map_name: str, museum_key: str, idx: int, total: int, chapter: str = "primitives") -> dict:
    ...
    # Added 2026-09-17 for Biome_Cage, whose one body is an 8 x 8 glass cage: the
    # colonnade knows a body only by its token cell, so four piers stood inside it.
    piers_ok = md.get("map_info", {}).get("museum", {}).get("piers", True)
...
def normalize_row(row: dict, post_color: bool, dirlocked: bool,
    ...
    if post_color:
        ...
        if interior_walls == 0 and H >= 8 and row.get("piers", True) is False:
            row["dressing"] = "piers: refused by map_info.museum.piers (bare map kept open)"
        elif interior_walls == 0 and H >= 8:
            near = set()
