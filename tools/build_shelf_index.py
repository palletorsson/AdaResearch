...
def main() -> int:
    ...
    for token, entry in sorted(caps.items()):
        ...
        if not angles:
            continue
        r = reg.get(token, {})
        # WHEN, so the shelf can answer "where is the last thing I made".
        # capture_multi_angle stamps each angle; the newest is the shoot.
        mt = (entry.get("mtimes") or {}).values()
        # A MAP IS NOT AN ARTIFACT, and the same tool photographs both into the
        # same directory. Without this the artifact shelf carries rooms —
        # Primitives_Melencolia, GT_Pathfinding — as "captures with no registry
        # entry", which is most of what that count was really measuring.
        is_map = (MAPS / token / "map_data.json").exists()
        rows.append({
            "token": token,
            ...
            "kind": "map" if is_map else "artifact",
            "placements": placed.get(token, 0),
            "captured_at": int(max(mt)) if mt else 0,
            "angles": angles,
            # the tile image; front if we have it, else whatever came back
            "tile": "front" if "front" in angles else angles[0],
        })

    arts = [r for r in rows if r["kind"] == "artifact"]
    unplaced = sum(1 for r in arts if r["placements"] == 0)
    # counted over ARTIFACTS only: a room has no registry entry by design
    orphan = sum(1 for r in arts if not r["in_registry"])
    n_maps_shot = sum(1 for r in rows if r["kind"] == "map")
    doc = {
    ...
    print("%d capture(s): %d artifacts, %d rooms — scanned %d maps" % (
        len(rows), len(arts), n_maps_shot, n_maps))
    print("  %d artifacts stand in no map at all" % unplaced)
    print("  %d artifacts have no registry entry behind the capture" % orphan)
    print("  -> %s" % OUT)
    return 0
