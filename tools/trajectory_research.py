#!/usr/bin/env python
"""
trajectory_research.py — Batch-render force-trajectory configs.
DNA = force + params + initial conditions + duration + interpretation.
"""
...
CONFIG_PATH = REPO_ROOT / "commons" / "trajectory_grammar" / "research_configs.json"
ENCYCLOPEDIA_DIR = REPO_ROOT.parent / "ada_encyclopedia"
OUTPUT_DIR = ENCYCLOPEDIA_DIR / "public" / "trajectory-gallery"
STAGE_DIR_NAME = "trajectory_gallery"
...
def render_one(config: dict, force: bool) -> bool:
    ...
    staging = REPO_ROOT / "commons" / "trajectory_grammar" / "_staging"
    ...
    res_cfg = f"res://commons/trajectory_grammar/_staging/{cid}.json"
    args = [
    ...
    print(f"  render {cid} ({config.get('force','?')}) ...")
    ...
    print(f"    -> public/trajectory-gallery/{cid}.png")
    return True
...
def main() -> int:
    ...
    print(f"Trajectory: {len(configs)} configs -> {OUTPUT_DIR}")
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    ok, fail = [], []
    for cfg in configs:
    ...
    for cfg in lib["configs"]:
        ...
        entries.append({
            ...
            "notes": cfg.get("notes", ""),
            "force": cfg.get("force", ""),
            "interpretation": cfg.get("interpretation", "tube"),
            "duration": cfg.get("duration", 0),
            "image": f"/trajectory-gallery/{cfg['id']}.png",
            "config": f"/trajectory-gallery/{cfg['id']}.json",
        })
    manifest = {
