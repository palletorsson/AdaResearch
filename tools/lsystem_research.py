#!/usr/bin/env python
"""
lsystem_research.py — Batch-render L-system configs.
DNA = axiom + rules + iterations + interpretation. The interpretation field
is the cross-substrate bridge: same L-system string renders via lines, tubes,
graph, softbody, or primitives depending on config.
"""
...
CONFIG_PATH = REPO_ROOT / "commons" / "lsystem_grammar" / "research_configs.json"
ENCYCLOPEDIA_DIR = REPO_ROOT.parent / "ada_encyclopedia"
OUTPUT_DIR = ENCYCLOPEDIA_DIR / "public" / "lsystem-gallery"
STAGE_DIR_NAME = "ls_gallery"
...
def render_one(config: dict, force: bool) -> bool:
    ...
    staging = REPO_ROOT / "commons" / "lsystem_grammar" / "_staging"
    ...
    res_cfg = f"res://commons/lsystem_grammar/_staging/{cid}.json"
    args = [
    ...
    print(f"  render {cid} ({config.get('interpretation','lines')}) ...")
    ...
    print(f"    -> public/lsystem-gallery/{cid}.png")
    return True
...
def main() -> int:
    ...
    print(f"L-system: {len(configs)} configs -> {OUTPUT_DIR}")
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    ok, fail = [], []
    for cfg in configs:
    ...
    for cfg in lib["configs"]:
        ...
        entries.append({
            ...
            "interpretation": cfg.get("interpretation", "lines"),
            "dna_connections": cfg.get("dna_connections", []),
            "image": f"/lsystem-gallery/{cfg['id']}.png",
            "config": f"/lsystem-gallery/{cfg['id']}.json",
        })
    manifest = {
