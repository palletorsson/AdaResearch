#!/usr/bin/env python
"""
soft_body_research.py — Batch-render soft body configs.
Loads commons/soft_body/research_configs.json, simulates each via Godot CLI,
copies PNG to ada_encyclopedia/public/soft-body-gallery/ with paired JSON.
"""
...
def render_one(config: dict, force: bool) -> bool:
    ...
    staging = REPO_ROOT / "commons" / "soft_body" / "_staging"
    ...
    res_cfg = f"res://commons/soft_body/_staging/{cid}.json"
    args = [
    ...
    print(f"    -> public/soft-body-gallery/{cid}.png")
    return True
...
def main() -> int:
    ...
    print(f"Soft body: {len(configs)} configs -> {OUTPUT_DIR}")
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    ok, fail = [], []
    for cfg in configs:
