...
def pearl_walks(chapter: str, trunk: dict[str, Any] | None = None) -> list[dict[str, Any]]:
    ...
    for pi, p in enumerate(pearls_all):
        ...
        walks.append({"chapter": chapter, "pearl": name, "pearl_index": int(p.get("index", len(walks))), "map": p.get("map", ""), "stages": list(p.get("stages", [])), "rooms": p.get("rooms"), "exclude": list(p.get("excluded", [])),
