...
def write_manifest(entries: list[dict], dest_root: Path, dry_run: bool, merge: bool = False) -> Path:
    """Write the manifest. With merge=True (a --token run) the synced tokens are
    UPDATED in the manifest that is already there; without it the manifest is
    rebuilt from this run alone. 2026-09-05: `--token comment_box` used to
    rewrite the 983-entry manifest down to ONE entry and blank every other
    artifact's face in the encyclopedia."""
    manifest_path = dest_root / "manifest.json"
    captures = {}
    if merge and manifest_path.exists():
        try:
            captures = dict(json.loads(manifest_path.read_text(encoding="utf-8")).get("captures", {}))
        except (OSError, ValueError):
            captures = {}
    for e in entries:
        captures[e["token"]] = {"angles": e["angles"], "mtimes": e["mtimes"]}
    manifest = {
...
def main() -> int:
    ...
    mpath = write_manifest(entries, dest_root, args.dry_run, merge=bool(args.token))
