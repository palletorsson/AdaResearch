...
func generate_line_paths() -> void:
    ...
    var rows := max(1, int(floor(span / row_spacing)) + 1)

    for r in range(rows):
        var lerp_t := 0.0 if rows == 1 else float(r) / float(rows - 1)
        var z_pos := lerp(start_z, end_z, lerp_t)
        var spans_in_row := 2 + (r % 2)
...
func _create_line_mesh_from_path(path: PackedVector3Array) -> ArrayMesh:
    ...
    var segments := max(3, ring_segments)
    var radius := max(0.01, line_radius)
...
func start_line_animations() -> void:
    ...
    if animation_speed > 0.0:
        var duration := 15.0 / max(animation_speed, 0.01)
...
func _animate_shader_properties() -> void:
    for material in line_materials:
        ...
        if abs(flow_speed) > 0.001:
            var base_flow := abs(flow_speed)
            var flow_min := base_flow * 0.5
            var flow_max := base_flow * 2.0
